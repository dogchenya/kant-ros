...
bool KT_OpenSSL::_initialize = false;
//////////////////////////////////////////////////////////////////////////////////////////

KT_OpenSSL::KT_OpenSSL(SSL* ssl) : _ssl(ssl), _bHandshaked(false), _isServer(false), _err(0), _plainBuf(NULL) {}

KT_OpenSSL::~KT_OpenSSL() { release(); }

void KT_OpenSSL::release() {
  if (_ssl) {
    SSL_free(_ssl);
    _ssl = NULL;
  }
...
void KT_OpenSSL::init(bool isServer) {
  _bHandshaked = false;
  _isServer = isServer;
  _err = 0;
}

std::string KT_OpenSSL::getErrMsg() const {
...
void KT_OpenSSL::setReadBufferSize(size_t size) { BIO_set_read_buffer_size(SSL_get_rbio(_ssl), size); }

void KT_OpenSSL::setWriteBufferSize(size_t size) { BIO_set_write_buffer_size(SSL_get_rbio(_ssl), size); }
...
shared_ptr<KT_OpenSSL::CTX> KT_OpenSSL::newCtx(const std::string& cafile, const std::string& certfile,
                                               const std::string& keyfile, bool verifyClient, const string& ciphers) {
  initialize();
...
#define RETURN_IF_FAIL(call)     \
  ...
  if (verifyClient) mode |= SSL_VERIFY_FAIL_IF_NO_PEER_CERT;
  if (!cafile.empty()) mode |= SSL_VERIFY_PEER;
  ...
  if (!cafile.empty()) RETURN_IF_FAIL(SSL_CTX_load_verify_locations(ctx, cafile.data(), NULL));

  // 客户端可以不提供证书的
  if (!certfile.empty()) RETURN_IF_FAIL(SSL_CTX_use_certificate_file(ctx, certfile.data(), SSL_FILETYPE_PEM));
...
#undef RETURN_IF_FAIL
  ...
  return std::make_shared<KT_OpenSSL::CTX>(ctx);
}

shared_ptr<KT_OpenSSL> KT_OpenSSL::newSSL(const std::shared_ptr<KT_OpenSSL::CTX>& ctx) {
  initialize();
  ...
  return std::make_shared<KT_OpenSSL>(ssl);
}
