...
static const unsigned short bytebit[8] = {0200, 0100, 040, 020, 010, 04, 02, 01};

static const uint32_t bigbyte[24] = {0x800000L, 0x400000L, 0x200000L, 0x100000L, 0x80000L, 0x40000L, 0x20000L, 0x10000L,
                                     0x8000L,   0x4000L,   0x2000L,   0x1000L,   0x800L,   0x400L,   0x200L,   0x100L,
                                     0x80L,     0x40L,     0x20L,     0x10L,     0x8L,     0x4L,     0x2L,     0x1L};

static const unsigned char pc1[56] = {56, 48, 40, 32, 24, 16, 8,  0,  57, 49, 41, 33, 25, 17, 9,  1,  58, 50, 42,
...
static const unsigned char pc2[48] = {13, 16, 10, 23, 0,  4,  2,  27, 14, 5,  20, 9,  22, 18, 11, 3,
...
void KT_Des::scrunch(register const char *outof, register uint32_t *into) {
  *into = (*outof++ & 0xffL) << 24;
  *into |= (*outof++ & 0xffL) << 16;
  *into |= (*outof++ & 0xffL) << 8;
  *into++ |= (*outof++ & 0xffL);
...
void KT_Des::unscrun(register uint32_t *outof, register char *into) {
  *into++ = (*outof >> 24) & 0xffL;
  *into++ = (*outof >> 16) & 0xffL;
  *into++ = (*outof >> 8) & 0xffL;
  *into++ = *outof++ & 0xffL;
...
void KT_Des::desfunc(register uint32_t *block, register uint32_t *keys) {
...
void KT_Des::cookey(register uint32_t *raw1, uint32_t *k) {
...
void KT_Des::deskey(const char *key, short edf, uint32_t *k) {
...
void KT_Des::des(const char *inblock, char *outblock, uint32_t *KnL) {
  uint32_t work[2];
...
void KT_Des::des3key(const char *hexkey, short mode, uint32_t *KnL, uint32_t *KnR, uint32_t *Kn3) {
...
void KT_Des::des3(const char *from, char *into, uint32_t *KnL, uint32_t *KnR, uint32_t *Kn3) {
  uint32_t work[2];
...
string KT_Des::encrypt(const char *key, const char *sIn, size_t iInlen) {
  char des_key[8] = {0};
  char des_input[8] = {0};
  char des_output[8] = {0};
...
string KT_Des::decrypt(const char *key, const char *sIn, size_t iInlen) {
  char des_key[8] = {0};
  char des_input[8] = {0};
  char des_output[8] = {0};
  ...
  if (ret.length() > 0) {
    ...
    if (pad > 0 && pad <= (int)sizeof(des_output) && (size_t)pad <= ret.length()) {
      ...
      if (bOk) ret.erase(ret.end() - pad, ret.end());
    }
  }
...
string KT_Des::encrypt3(const char *key, const char *sIn, size_t iInlen) {
  char des_key[24] = {0};
...
string KT_Des::decrypt3(const char *key, const char *sIn, size_t iInlen) {
  try {
    char des_key[24] = {0};
    ...
    return ret;
  } catch (KT_DES_Exception &e) {
    string res = "KT_Des::decrypt3 error" + string(e.what());
    throw KT_DES_Exception(res);
  }
}
