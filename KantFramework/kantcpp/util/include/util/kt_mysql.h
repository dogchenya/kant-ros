...
*/
struct KT_Mysql_Exception : public KT_Exception {
  KT_Mysql_Exception(const string &sBuffer) : KT_Exception(sBuffer){};
  ~KT_Mysql_Exception() throw(){};
};
...
* 
* 非线程安全，通常一个线程一个KT_Mysql对象；
* Non-thread safe, usually one thread and one KT_Mysql object;
* 
* 对于insert/update可以有更好的函数封装，保证SQL注入；
* For insert/update better function encapsulation guarantees SQL injection:
* 
* KT_Mysql::DB_INT表示组装sql语句时，不加””和转义；
* KT_Mysql:: DB_INT means no "" and escape when assembling SQL statements;
* 
* KT_Mysql::DB_STR表示组装sql语句时，加””并转义；
* KT_Mysql:: DB_STR means to add "" and escape when assembling SQL statements;
*/
class KT_Mysql {
 public:
  /**
  ...
  /**
    ...
    */
  KT_Mysql(const string &sHost, const string &sUser = "", const string &sPasswd = "", const string &sDatabase = "",
  ...
  /**
    ...
    */
  KT_Mysql(const KT_DBConf &tcDBConf);
  ...
  /**
    ...
    * @param tcDBConf Database Configuration
    */
  void init(const KT_DBConf &tcDBConf);
  ...
  /**
    ...
    * @param sSql sql语句
    * @param sSql SQL statement
    * @param pdakantunc ，函数参数为map<string,string> ,key为column 名，value为数据
    * @param pdakantunc , Function parameter is map<string, string>, key is column name, value is data
    * @throws KT_Mysql_Exception
    * @return MysqlData类型的数据，可以根据字段获取相关信息
    * @return MysqlData type of data, you can get information based on the field
    */
  size_t travelRecord(const string &sSql, const std::function<void(const map<string, string> &)> &pdakantunc);
  ...
  /**
    ...
    */
  size_t updateRecord(const string &sTableName, const map<string, pair<FT, string>> &mpColumns,
  ...
  /**
    ...
    */
  string buildUpdateSQL(const string &sTableName, const map<string, pair<FT, string>> &mpColumns,
                        const string &sCondition);
  ...
  /**
    ...
    * @return            string Update-SQL statement
    */
  static string buildUpdateSQLNoSafe(const string &sTableName, const map<string, pair<FT, string>> &mpColumns,
                                     const string &sCondition);
 ...
 protected:
  /**
    ...
    */
  KT_Mysql(const KT_Mysql &tcMysql);
  ...
  /**
    ...
    */
  KT_Mysql &operator=(const KT_Mysql &tcMysql);
  ...
  string buildBatchSQL(const string &sTableName, const string &command,
                       const map<string, pair<FT, vector<string>>> &mpColumns);
  static string buildSQLNoSafe(const string &sTableName, const string &command,
                               const map<string, pair<FT, string>> &mpColumns);
  static string buildBatchSQLNoSafe(const string &sTableName, const string &command,
                                    const map<string, pair<FT, vector<string>>> &mpColumns);
