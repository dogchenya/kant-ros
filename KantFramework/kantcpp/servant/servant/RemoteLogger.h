#ifndef _KANT_LOGGER_H_
#define _KANT_LOGGER_H_
...
#include "util/kt_thread_rwlock.h"
#include "util/kt_singleton.h"

#define DYEING_DIR "kant_dyeing"
...
/*****************************************************************************
实现方式说明(只介绍按时间的日志, 会写到kantlog):
    1 自定义时间日志的WriteT类:RemoteTimeWriteT
    2 在RemoteTimeWriteT类中, 写入到远程
    3 定义远程日志类:typedef KT_Logger<RemoteTimeWriteT, KT_RollByTime> RemoteTimeLogger;
...
class RollWriteT {
 public:
  ...
  void setDyeingLogInfo(const string &sApp, const string &sServer, const string &sLogPath, int iMaxSize, int iMaxNum,
                        const LogPrx &logPrx, const string &sLogObj);

 protected:
  KT_RollLogger *_dyeingRollLogger;
 ...
 * 当该对象析够以后, 则直接cout出来
 */
class SVT_DLL_API LocalRollLogger : public KT_Singleton<LocalRollLogger, CreateUsingNew, PhoneixLifetime> {
 public:
  enum {
    ...
    KANT_LOG = 6   /**写错误,警告,调试,Info log*/
  };

  typedef KT_Logger<RollWriteT, KT_RollBySize> RollLogger;
  ...
  /**
     ...
     * @param iMaxNum, 文件最大数
     */
  void setLogInfo(const string &sApp, const string &sServer, const string &sLogpath, int iMaxSize = 1024 * 1024 * 50,
                  int iMaxNum = 10, const CommunicatorPtr &comm = NULL, const string &sLogObj = "");
 ...
 protected:
  ...
  /**
     * lock
     */
  KT_ThreadRWLocker _mutex;

  /**
     * 本地线程组
     */
  KT_LoggerThreadGroup _local;
 ...
 * 作为单件存在, 且是永生不死的单件
 */
class SVT_DLL_API KantLoggerThread : public KT_Singleton<KantLoggerThread, CreateUsingNew, PhoneixLifetime> {
 public:
  /**
     * 构造函数
     */
  ...
  /**
     * 本地写日志线程
     */
  KT_LoggerThreadGroup *local();

  /**
     * 远程写日志线程
     *
     * @return KT_LoggerThreadGroup*
     */
  KT_LoggerThreadGroup *remote();
...
class TimeWriteT {
 public:
  typedef KT_Logger<RemoteTimeWriteT, KT_RollByTime> RemoteTimeLogger;

  typedef KT_Logger<KT_DefaultWriteT, KT_RollByTime> DyeingTimeLogger;
  ...
  /**
     ...
     */
  void setLogInfo(const LogPrx &logPrx, const string &sApp, const string &sServer, const string &sFile,
                  const string &sLogpath, const string &sFormat, const string &setdivision = "",
                  const string &sLogType = "", const PropertyReportPtr &reportSuccPtr = NULL,
                  const PropertyReportPtr &reportFailPtr = NULL);
 ...
 protected:
  ...
  /**
     * 错误文件
     */
  KT_DayLogger _logger;

  /**
     * 缺省写模式
     */
  KT_DefaultWriteT _wt;
 ...
 * 远程日志帮助类, 单件
 */
class SVT_DLL_API RemoteTimeLogger : public KT_ThreadLock,
                                     public KT_Singleton<RemoteTimeLogger, CreateUsingNew, DefaultLifetime> {
 public:
  //定义按时间滚动的日志
  typedef KT_Logger<TimeWriteT, KT_RollByTime> TimeLogger;
  ...
  /**
     ...
     * @param logpath, 日志路径
     */
  void setLogInfo(const CommunicatorPtr &comm, const string &obj, const string &sApp, const string &sServer,
                  const string &sLogpath, const string &setdivision = "", const bool &bLogStatReport = false);
  ...
  void initFormat(const string &sApp, const string &sServer, const string &sFile, const string &sFormat,
                  const LogTypePtr &logTypePtr = NULL);
  /**
     * 初始化设置时间格式("%Y%m%d")
     * 不要动态修改, 线程不安全
     * 如果有需要, 初始化后直接修改
     * @param sFormat, 文件名称, 为空表示缺省的时间日志
     * @param frequency 支持每多少天/小时/分钟，详见KT_logger.h中关于KantLogByDay,KantLogByHour,KantLogByMinute的描述
     *
     * 用法: 按两个小时记录日志
     * initFormat<KantLogByHour>("logfile",KantLogByHour::FORMAT,2);
     */
  ...
  template <class TLogType>
  void initFormatWithType(const string &sApp, const string &sServer, const string &sFile, const string &sFormat,
                          size_t frequency) {
    LogTypePtr logTypePtr = new TLogType(sFormat, frequency);
 ...
 protected:
  ...
  void initTimeLogger(TimeLogger *pTimeLogger, const string &sApp, const string &sServer, const string &sFile,
                      const string &sFormat, const LogTypePtr &logTypePtr = NULL);
 ...
 * 染色开关类，析构时关闭
 */
class KantDyeingSwitch {
 public:
  /**
     * 构造函数，默认不打开染色日志
     */
  KantDyeingSwitch() : _needDyeing(false) {}
...
#define LOGMSG(level, ...)                                     \
  do {                                                         \
    if (LOG->isNeedLog(level)) LOG->log(level) << __VA_ARGS__; \
  } while (0)
#define LOG_MSG(level, ...)                                                             \
  do {                                                                                  \
    if (LOG->isNeedLog(level)) LOG->log(level) << FILE_FUNC_LINE << "|" << __VA_ARGS__; \
  } while (0)

#define LOGEXMSG(x, level, ...)                            \
  do {                                                     \
    if (x->isNeedLog(level)) x->log(level) << __VA_ARGS__; \
  } while (0)
#define LOGEX_MSG(x, level, ...)                                                    \
  do {                                                                              \
    if (x->isNeedLog(level)) x->log(level) << FILE_FUNC_LINE << "|" << __VA_ARGS__; \
  } while (0)

#else
#define LOGMSG(level, msg...)                          \
  do {                                                 \
    if (LOG->isNeedLog(level)) LOG->log(level) << msg; \
  } while (0)
#define LOG_MSG(level, msg...)                                                  \
  do {                                                                          \
    if (LOG->isNeedLog(level)) LOG->log(level) << FILE_FUNC_LINE << "|" << msg; \
  } while (0)

#define LOGEXMSG(x, level, msg...)                 \
  do {                                             \
    if (x->isNeedLog(level)) x->log(level) << msg; \
  } while (0)
#define LOGEX_MSG(x, level, msg...)                                         \
  do {                                                                      \
    if (x->isNeedLog(level)) x->log(level) << FILE_FUNC_LINE << "|" << msg; \
  } while (0)
 ...
 */
#if TARGET_PLATFORM_WINDOWS
#define TLOGKANT(...) LOGMSG(LocalRollLogger::KANT_LOG, __VA_ARGS__)
...
#define TLOG_KANT(...) LOG_MSG(LocalRollLogger::KANT_LOG, __VA_ARGS__)
...
#define TLOGEXKANT(x, ...) LOGEXMSG(LOG_EX(x), LocalRollLogger::KANT_LOG, __VA_ARGS__)
#define TLOGEXKANT(x, ...) LOGEXMSG(LOG_EX(x), LocalRollLogger::KANT_LOG, __VA_ARGS__)
...
#define TLOGEX_KANT(x, ...) LOGEX_MSG(LOG_EX(x), LocalRollLogger::KANT_LOG, __VA_ARGS__)
#define TLOGEX_KANT(x, ...) LOGEX_MSG(LOG_EX(x), LocalRollLogger::KANT_LOG, __VA_ARGS__)
...
#else
#define TLOGKANT(msg...) LOGMSG(LocalRollLogger::KANT_LOG, msg)
...
#define TLOG_KANT(msg...) LOG_MSG(LocalRollLogger::KANT_LOG, msg)
...
#define TLOGEXKANT(x, msg...) LOGEXMSG(LOG_EX(x), LocalRollLogger::KANT_LOG, msg)
...
#define TLOGEX_KANT(x, msg...) LOGEX_MSG(LOG_EX(x), LocalRollLogger::KANT_LOG, msg)
...
#define KANT_TRACE(traceKey, annotation, client, server, func, ret, data, ex)                                          \
  {                                                                                                                    \
    FDLOG(TRACE_LOG_FILENAME) << traceKey << "|" << annotation << "|" << client << "|" << server << "|" << func << "|" \
                              << TNOWMS << "|" << ret << "|" << KT_Base64::encode(data) << "|" << ex << endl;          \
  }
//////////////////////////////////////////////
 ...
 */
#define TENREMOTE_FDLOG(swith, sApp, sServer, sFile) \
  (RemoteTimeLogger::getInstance()->enableRemoteEx(sApp, sServer, sFile, swith))
#define TENLOCAL_FDLOG(swith, sApp, sServer, sFile) \
  (RemoteTimeLogger::getInstance()->enableLocalEx(sApp, sServer, sFile, swith))
