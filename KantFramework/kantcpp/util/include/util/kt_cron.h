...
namespace kant {
struct KT_CronException : public KT_Exception {
 public:
  KT_CronException(const string& buffer) : KT_Exception(buffer){};
  //	KT_CronException(const string& buffer, bool err) : KT_Exception(buffer, err) {};
  ~KT_CronException() throw(){};
};
...
class KT_Cron {
 public:
  enum class CronField { second, minute, hour_of_day, day_of_week, day_of_month, month, year };
 ...
 public:
  ...
  static KT_Cron makecron(const string& expr);
  ...
  /**
     ...
     */
  static std::time_t nextcron(const KT_Cron& cron, std::time_t timestamp);
  ...
  /**
     ...
     * @return If INVALID_TIME is returned, it is the wrong time. Otherwise the correct timestamp is returned.
     */
  static std::time_t nextcron(const KT_Cron& cron);
 ...
 protected:
  ...
  /**
    ...
    */
  template <size_t N>
  static size_t findNext(const std::bitset<N>& target, std::tm& tmt, unsigned int minimum, unsigned int maximum,
                         unsigned int value, CronField field, CronField next_field,
                         const std::bitset<7>& marked_fields);
  static bool findNext(const KT_Cron& cron, std::tm& tmt, size_t dot);
  static size_t findNextDay(std::tm& tmt, std::bitset<31> const& days_of_month, size_t day_of_month,
...
template <size_t N>
void KT_Cron::setCronField(std::string value, std::bitset<N>& target, cron_int minval, cron_int maxval) {
  if (value.length() > 0 && value[value.length() - 1] == ',')
    throw KT_CronException("[KT_Cron::setCronField] value cannot end with comma");

  auto fields = KT_Common::sepstr<string>(value, ",");
  if (fields.empty()) throw KT_CronException("[KT_Cron::setCronField] expression parsing error");
  ...
  for (auto const& field : fields) {
    if (!contains(field, '/')) {
      ...
      if (parts.size() != 2) throw KT_CronException("[KT_Cron::setCronField] incrementer must have two fields");
      ...
      if (delta <= 0) throw KT_CronException("[KT_Cron::setCronField] incrementer must be a positive value");
...
template <size_t N>
size_t KT_Cron::nextSetbit(const std::bitset<N>& target, size_t minimum, size_t maximum, size_t offset) {
...
template <size_t N>
size_t KT_Cron::findNext(const std::bitset<N>& target, std::tm& tmt, unsigned int minimum, unsigned int maximum,
                         unsigned int value, CronField field, CronField next_field,
                         const std::bitset<7>& marked_fields) {
