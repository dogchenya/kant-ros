 ...
 * 使用说明:
 * 设定函数的定时时间, 定时时间到, 则回调
 * 可以设定回调线程数, 默认只有一个, 底层用kt_thread_pool实现线程池的
 * 任何可以丢进kt_thread_pool的function对象(std::bind的对象), 都可以丢该kt_timer
 *
 */
/////////////////////////////////////////////////
...
class KT_TimerBase {
 ...
 public:
  ...
  int64_t postDelayed(int64_t delayMillseconds, F &&f, Args &&... args) {
    uint64_t fireMillseconds = KT_TimeProvider::getInstance()->getNowMs() + delayMillseconds;
  ...
  int64_t postRepeated(int64_t repeatTime, bool execNow, F &&f, Args &&... args) {
    ...
    if (execNow) {
      fireMillseconds = KT_TimeProvider::getInstance()->getNowMs();
    } else {
      fireMillseconds = KT_TimeProvider::getInstance()->getNowMs() + repeatTime;
    }
 ...
 protected:
  ...
  shared_ptr<Func> create(int64_t fireMillseconds, int64_t repeatTime, const string &cronexpr, F &&f, Args &&... args) {
    ...
    if (!cronexpr.empty()) {
      fPtr->_cron = KT_Cron::makecron(cronexpr);
      fPtr->_fireMillseconds = KT_Cron::nextcron(fPtr->_cron) * 1000;
    }
      ...
      fPtr->_func = [task, this, wPtr, repeatTime]() {
        ...
        if (p) {
          ...
          if (this->exist(p->_uniqueId, true)) {
            if (p->_cron.isset) {
              p->_fireMillseconds = KT_Cron::nextcron(p->_cron, p->_fireMillseconds / 1000) * 1000;
              this->post(p);
            } else if (repeatTime > 0) {
              p->_fireMillseconds = KT_TimeProvider::getInstance()->getNowMs() + repeatTime;
              this->post(p);
            }
          }
        }
 ...
 * @brief  定时器类
 */
class KT_Timer : public KT_TimerBase {
 public:
  /**
	 * 析构
	 */
