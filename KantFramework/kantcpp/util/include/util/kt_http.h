 ...
 * @brief  http class
 *  
 * 包括KT_HttpRequest、KT_HttpResponse两个类；
 * Including KT_HttpRequest、KT_HttpResponse Two classes of httpresponse;
 *  
...
*/
struct KT_Http_Exception : public KT_Exception {
  KT_Http_Exception(const string &sBuffer) : KT_Exception(sBuffer){};
  ~KT_Http_Exception(){};
};
 ...
 */
struct KT_HttpRequest_Exception : public KT_Http_Exception {
  KT_HttpRequest_Exception(const string &sBuffer) : KT_Http_Exception(sBuffer){};
  ~KT_HttpRequest_Exception(){};
};

class KT_TCPClient;
class KT_HttpRequest;
class KT_HttpResponse;
...
class KT_URL {
 public:
  /**
     ...
     *  @brief  URL type
     */
  enum URL_TYPE { HTTP, HTTPS, FTP };

  /**
     *  @brief  构造函数
     *  @brief  Constructor
     */
  KT_URL() : _iURLType(HTTP) {}
  ...
  /** 
     ...
     */
  KT_URL buildWithRelativePath(const string &sRelativeURL) const;
...
class KT_Http {
 public:
  ...
  /**
     ...
     */
  void setContentLength(size_t iContentLength) { setHeader("Content-Length", KT_Common::tostr(iContentLength)); }
  ...
  void appendContent(const char *buff, size_t len, bool bUpdateContentLength = false) {
    ...
    if (bUpdateContentLength) {
      ...
      if (_content.length() > 0) setContentLength(_content.length());
    }
  }
...
class KT_HttpCookie {
 public:
  typedef map<string, string, KT_Http::CmpCase> http_cookie_data;
 ...
 protected:
  /**
     ...
     * @return size_t
     */
  size_t isCookieMatch(const Cookie &cookie, const KT_URL &tURL) const;
...
class KT_HttpResponse : public KT_Http {
 public:
  /**
     * @brief 构造
     * @brief Construct
     */
  KT_HttpResponse() { KT_HttpResponse::reset(); }
  ...
  /**
     ...
     * (能够解析的数据KT_HttpResponse会自动从sBuffer里面消除，直到网络接收完毕或者解析返回true)
     ...
     */
  bool incrementDecode(KT_NetWorkBuffer &buff);
  ...
  /**
	 ...
	 *         Or you may not need to parse any more
	 */
  bool incrementDecode(KT_NetWorkBuffer::Buffer &buff);
  ...
  //
  //		_about      = KT_Common::trim(string(f2 + 1, it));
  //
...
class KT_HttpRequest : public KT_Http {
 public:
  ///////////////////////////////////////////////////////////////////
  KT_HttpRequest() {
  ...
  /**
     ...
     * @throws KT_HttpRequest_Exception, 不支持的http协议, 抛出异常
     * @throws KT_HttpRequest_Exception, unsupported http protocol, throw exception
  ...
  /**
     ...
     * @return
     */
  static bool checkRequest(KT_NetWorkBuffer &buff);
  ...
  /**
     ...
     * @throw         KT_HttpRequest_Exception 
     */
  bool decode(const string &sBuffer);
  ...
  /**
     * encode buffer to KT_NetWorkBuffer
     * @param buff
     */
  void encode(shared_ptr<KT_NetWorkBuffer::Buffer> &buff);

  /**
     * encode buffer to KT_NetWorkBuffer
     * @param buff
     */
  void encode(KT_NetWorkBuffer &buff);
  ...
  /**
     ...
     * @return
     */
  int doRequest(KT_TCPClient &client, KT_HttpResponse &stHttpRsp);
  ...
  /**
     ...
     * @return const KT_URL&
     */
  const KT_URL &getURL() const { return _httpURL; }
