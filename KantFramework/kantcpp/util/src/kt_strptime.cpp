...
static char weekday_name[][20] = {"Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"};
static char ab_weekday_name[][10] = {"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"};
static char month_name[][20] = {"January", "February", "March",     "April",   "May",      "June",
                                "July",    "August",   "September", "October", "November", "December"};
static char ab_month_name[][10] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};
...
void day_of_the_week(struct tm *tm) {
  ...
  int wday = (-473 + (365 * (tm->tm_year - 70)) + (corr_year / 4) - ((corr_year / 4) / 25) +
              ((corr_year / 4) % 25 < 0) + (((corr_year / 4) / 25) / 4) + __mon_yday[0][tm->tm_mon] + tm->tm_mday - 1);
...
char *strptime_internal(const char *rp, const char *fmt, struct tm *tm, enum locale_status *decided) {
  ...
  start_over:
    ...
    switch (*fmt++) {
      ...
      case 'A':
        ...
        if (cnt == 7) /* Does not match a weekday name.  */
          return NULL;
        tm->tm_wday = cnt;
        have_wday = 1;
        break;
      ...
      case 'h':
        ...
        if (cnt == 12) /* Does not match a month name.  */
          return NULL;
        tm->tm_mon = cnt;
        want_xday = 1;
        break;
      case 'c':
        ...
        if (!recursive(HERE_T_FMT_AMPM)) return NULL;
        break;
      case 'C':
      ...
      case 'e':
        ...
        break;
      case 'F':
        if (!recursive("%Y-%m-%d")) return NULL;
        want_xday = 1;
        break;
      case 'x':
        /* Fall through.  */
      case 'D':
        ...
        if (!recursive(HERE_D_FMT)) return NULL;
        want_xday = 1;
        break;
      case 'k':
      case 'H':
      ...
      case 's': {
        ...
        if ((gmtime_s(tm, &secs)) == 0) /* Error in function.  */
          return NULL;
      } break;
      case 'S':
        get_number(0, 61, 2);
      ...
      case 'X':
        /* Fall through.  */
      case 'T':
        if (!recursive(HERE_T_FMT)) return NULL;
        break;
      case 'u':
        get_number(1, 7, 1);
        tm->tm_wday = val % 7;
  ...
  if (want_xday && !have_wday) {
    if (!(have_mon && have_mday) && have_yday) {
      ...
      int t_mon = 0;
      while (__mon_yday[__isleap(1900 + tm->tm_year)][t_mon] <= tm->tm_yday) t_mon++;
      if (!have_mon) tm->tm_mon = t_mon - 1;
      if (!have_mday) tm->tm_mday = tm->tm_yday - __mon_yday[__isleap(1900 + tm->tm_year)][t_mon - 1] + 1;
    }
    day_of_the_week(tm);
  }
  if (want_xday && !have_yday) day_of_the_year(tm);
