#ifndef _KT_LOGGER_H_
#define _KT_LOGGER_H_
...
/**
	 ...
	 *
	 *  1 实现自定义模板类,继承于KT_LoggerRoll,实现roll函数,该函数实现写逻辑,
	 *  1 Implement a custom template class that inherits from KT_LoggerRoll, which implements the roll function, which implements write logic
	 *
	 *  例如:template<typename WriteT>
	 *  for example: template<typename WriteT>
	 *
	 *  class MyRoll : public KT_LoggerRoll, public KT_ThreadMutex
	 *
	 *  2 模板中的WriteT是具体的写入逻辑,提供缺省的:KT_DefaultWriteT
	 *  2 WriteT in the template is the specific write logic that provides the default: KT_DefaultWriteT
	 *
	 ...
	 *
	 *  5 RollWrapperI主要就是提供MyRoll的初始化接口, 这些接口最后会直接反应到KT_Logger中.
	 *  5 RollWrapperI mainly provides MyRoll's initialization interfaces, which will eventually be reflected directly in KT_Logger.
	 *
	 ...
	 *
	 *  typedef KT_Logger<MyWriteT, MyRoll> MyLogger;
	 *
...
/**
	...
	*/
struct KT_Logger_Exception : public KT_Exception {
  KT_Logger_Exception(const string &buffer) : KT_Exception(buffer){};
  KT_Logger_Exception(const string &buffer, int err) : KT_Exception(buffer, err){};
  ~KT_Logger_Exception() throw(){};
};
...
class UTIL_DLL_API KT_LoggerRoll : public std::enable_shared_from_this<KT_LoggerRoll> {
 public:
  /**
		 * @brief 构造函数
		 * @brief Constructor
		 */
  KT_LoggerRoll() : _pThreadGroup(NULL) {}
  ...
  /**
		 ...
		 * @param ltg
		 */
  void setupThread(KT_LoggerThreadGroup *pThreadGroup);
  ...
  void enableDyeing(bool bEnable, const string &sKey) {
    ...
    if (bEnable) {
      _mapThreadID.insert(std::make_pair(KT_Thread::CURRENT_THREADID(), sKey));
      //_setThreadID.insert(pthread_self());
    } else {
      _mapThreadID.erase(KT_Thread::CURRENT_THREADID());
 ...
 protected:
  /**
		 * buffer
		 */
  KT_CasQueue<pair<std::size_t, string>> _buffer;
  ...
  /**
		 ...
		 * Thread Group
		 */
  KT_LoggerThreadGroup *_pThreadGroup;
  ...
  static KT_SpinLock _mutexDyeing;
...
class LoggerStream {
 public:
  /**
		 ...
		 * @param mutex
		 */
  LoggerStream(const char *header, ostream *stream, ostream *estream, KT_ThreadMutex &mutex)
    : _stream(stream), _estream(estream), _mutex(mutex) {
    if (stream) {
      _buffer << header;
    }
  }
...
class KT_Logger : public RollPolicy<WriteT>::RollWrapperI {
 protected:
  /**
		 * @brief 日志级别名称
 ...
 public:
  ...
  bool isNeedLog(int level) { return level <= _level; }
  /**
 ...
 protected:
  ...
  void head(char *c, int len, int level) {
    ...
    if (hasFlag(KT_Logger::HAS_MTIME)) {
      ...
      const char *szFormat =
        (_bHasSquareBracket) ? ("[%04d-%02d-%02d %02d:%02d:%02d.%03ld]%s") : ("%04d-%02d-%02d %02d:%02d:%02d.%03ld%s");
      n += snprintf(c + n, len - n, szFormat, tt.tm_year + 1900, tt.tm_mon + 1, tt.tm_mday, tt.tm_hour, tt.tm_min,
                    tt.tm_sec, duration_in_ms % 1000, _sSepar.c_str());
    } else if (hasFlag(KT_Logger::HAS_TIME)) {
      ...
      // #endif
      const char *szFormat =
        (_bHasSquareBracket) ? ("[%04d-%02d-%02d %02d:%02d:%02d]%s") : ("%04d-%02d-%02d %02d:%02d:%02d%s");
      n += snprintf(c + n, len - n, szFormat, tt.tm_year + 1900, tt.tm_mon + 1, tt.tm_mday, tt.tm_hour, tt.tm_min,
                    tt.tm_sec, _sSepar.c_str());
    }

    if (hasFlag(KT_Logger::HAS_PID)) {
      n += snprintf(c + n, len - n, "%zd%s", KT_Thread::CURRENT_THREADID(), _sSepar.c_str());
    }

    if (hasFlag(KT_Logger::HAS_LEVEL)) {
      n += snprintf(c + n, len - n, "%s%s", KT_Logger::LN[level].c_str(), _sSepar.c_str());
    }
  }
...
class RollWrapperBase : public RollWrapperInterface {
 public:
  typedef std::shared_ptr<RollPolicyWriteT> RollPolicyWriteTPtr;

  /**
		 * @brief 构造
		 * @brief Constructor
		 */
  RollWrapperBase() { _roll = std::make_shared<RollPolicyWriteT>(); }
  ...
  /**
		 ...
		 * @param ltg
		 */
  void setupThread(KT_LoggerThreadGroup *ltg) { _roll->setupThread(ltg); }
...
class KT_RollBySize : public KT_LoggerRoll, public std::mutex {
 public:
  typedef WriteT T;
  ...
  /**
		 ...
		 */
  class RollWrapperI : public RollWrapperBase<KT_RollBySize<WriteT>> {
   public:
    /**
			 * @brief 初始化.
			 * @brief Initialize
  ...
  /**
		 ...
		 * @brief Constructor
		 */
  KT_RollBySize() : _maxSize(5000000), _maxNum(10), _iUpdateCount(0), _lt(time(NULL)) {}
  ...
  void roll(const deque<pair<size_t, string>> &buffer) {
    ...
    if (!_of.is_open()) {
      ...
      string sLogFilePath = KT_File::extractFilePath(_path);

      if (!KT_File::isFileExist(sLogFilePath)) {
        KT_File::makeDirRecursive(sLogFilePath);
      }
      ...
      if (!_of.is_open()) {
        ...
        _t(_of, buffer);
        THROW_EXCEPTION_SYSCODE(KT_Logger_Exception, "[KT_RollBySize::roll]:fopen fail: " + sLogFileName);
        // throw KT_Logger_Exception("[KT_RollBySize::roll]:fopen fail: " + sLogFileName, KT_Exception::getSystemCode());
      }
    }
    ...
    string sLogFileName = _path + KT_Common::tostr(_maxNum - 1) + ".log";
    {
      if (KT_File::isFileExist(sLogFileName) && KT_File::removeFile(sLogFileName, true)) {
        return;
      }
    }
    ...
    for (int i = _maxNum - 2; i >= 0; i--) {
      if (i == 0) {
        ...
        sLogFileName = _path + KT_Common::tostr(i) + ".log";
      }

      if (KT_File::isFileExist(sLogFileName)) {
        string sNewLogFileName = _path + KT_Common::tostr(i + 1) + ".log";
...
typedef KT_Logger<KT_DefaultWriteT, KT_RollBySize> KT_RollLogger;
...
class LogType {
 public:
  LogType() : _next_time_t(0), _format("%Y%m%d"), _frequency(1), _des("day") {
    ...
    _next_cut_time = KT_Common::tm2str(_next_time_t, "%Y%m%d");
  }
  ...
  inline std::string &toString() { return _des; }
...
typedef std::shared_ptr<LogType> LogTypePtr;
...
class LogByDay : public LogType {
 ...
 public:
  explicit LogByDay(const string &format = "%Y%m%d", size_t frequency = 1) {
    ...
    _des = KT_Common::tostr(_frequency) + "day";
  }
  size_t frequence() {
...
class LogByHour : public LogType {
 ...
 public:
  explicit LogByHour(const string &format = "%Y%m%d%H", size_t frequency = 1) {
    init(format, frequency);
    _des = KT_Common::tostr(_frequency) + "hour";
  }
...
class LogByMinute : public LogType {
 ...
 public:
  explicit LogByMinute(const string &format = "%Y%m%d%H%M", size_t frequency = 1) {
    init(format, frequency);
    _des = KT_Common::tostr(_frequency) + "minute";
  }
  size_t frequence() {
...
/**
	 ...
	 */
template <typename WriteT>
class KT_RollByTime : public KT_LoggerRoll, public std::mutex {
 public:
  typedef WriteT T;
  ...
  /**
		 ...
		 */
  class RollWrapperI : public RollWrapperBase<KT_RollByTime<WriteT>> {
   public:
    /**
			 * @brief 初始化.
			 ...
			 */
    void init(const string &path, const string &format = "%Y%m%d", bool bHasSufix = true,
              const string &sConcatstr = "_", const LogTypePtr &logTypePtr = NULL, bool bIsRemote = false) {
  ...
  /**
		 ...
		 * @brief Construction
		 */
  KT_RollByTime() : _lt(0), _logTypePtr(NULL), _bRemoteType(false) {
  ...
  void init(const string &path, const string &format = "%Y%m%d", bool bHasSufix = true, const string &sConcatstr = "_",
            const LogTypePtr &logTypePtr = NULL, bool bIsRemote = false) {
    std::lock_guard<std::mutex> lock(*this);
    ...
    if (_logTypePtr) {
      ...
      _currentTime = KT_Common::now2str(_format);
    }
  }
  ...
  /**
		  ...
		  */
  void setRemote(bool en) { _bRemoteType = en; }
  /**
		 * @brief 获取写示例.
		 * @brief Get Write Samples
		 *
  ...
  void roll(const deque<pair<size_t, string>> &buffer) {
    ...
    if (_logTypePtr) {
      ...
      nowTime = KT_Common::tm2str(t, _format);
      //检查时间是否跨时间了, 跨时间后关闭文件
    ...
    if (!_of.is_open()) {
      ...
      if (!_of) {
        ...
        //Continue to _t for remote logging before throwing an exception
        _t(_of, buffer);
        THROW_EXCEPTION_SYSCODE(KT_Logger_Exception, "[KT_RollByTime::roll]:fopen fail: " + sLogFileName);
        // throw KT_Logger_Exception("[KT_RollByTime::roll]:fopen fail: " + sLogFileName, KT_Exception::getSystemCode());
      }
    }
...
typedef KT_Logger<KT_DefaultWriteT, KT_RollByTime> KT_DayLogger;

UTIL_DLL_API extern KT_RollLogger __global_logger_debug__;

#define LOG_CONSOLE_DEBUG                                                                             \
  __global_logger_debug__.any() << KT_Common::now2msstr() << "|" << std::this_thread::get_id() << "|" \
                                << FILE_FUNC_LINE << "|"
