...
TaskList::TaskList(const TaskReq &taskReq, unsigned int t) : _taskReq(taskReq) {
  //_adminPrx = Communicator::getInstance()->stringToProxy<AdminRegPrx>(g_pconf->get("/kant/objname<AdminRegObjName>", ""));
  ...
  _taskRsp.createTime = KT_Common::now2str("%Y-%m-%d %H:%M:%S");
  ...
  _createTime = KT_TimeProvider::getInstance()->getNow();
  _timeout = t;
  _finished = false;
}

bool TaskList::isTimeout() { return (_createTime + _timeout < TNOW); }
...
void TaskList::setRspInfo(size_t index, bool start, EMTaskItemStatus status) {
  ...
  map<string, string> info;
  {
    ...
    if (start) {
      _taskRsp.taskItemRsp[index].startTime = KT_Common::now2str("%Y-%m-%d %H:%M:%S");
      info["start_time"] = _taskRsp.taskItemRsp[index].startTime;
    } else {
      _taskRsp.taskItemRsp[index].endTime = KT_Common::now2str("%Y-%m-%d %H:%M:%S");
      info["end_time"] = _taskRsp.taskItemRsp[index].endTime;
    }

    _taskRsp.taskItemRsp[index].status = status;
    info["status"] = KT_Common::tostr(_taskRsp.taskItemRsp[index].status);
...
int TaskList::prepareFile() {
  ...
  for (auto e : pis) {
    ...
    TLOG_DEBUG("preparePatch_inner prepare :" << e.second.application << "." << e.second.serverName
                                              << ", patchId:" << e.second.patchId << ", file:" << e.second.patchFile
                                              << ", image:" << e.second.baseImage << endl);
...
EMTaskItemStatus TaskList::start(const TaskItemReq &req, string &log) {
  ...
  try {
    ...
    if (ret == 0) return EM_I_SUCCESS;
  } catch (exception &ex) {
    log = ex.what();
    TLOG_ERROR("TaskList::executeSingleTask startServer error:" << log << endl);
  }

  TLOG_DEBUG("TaskList::startServer error:" << req.application << "," << req.serverName << "," << req.nodeName << ","
                                            << req.userName << ", info:" << log << endl);
...
EMTaskItemStatus TaskList::restart(const TaskItemReq &req, string &log) {
  ...
  TLOG_ERROR("TaskList::restartServer error:" << req.application << "," << req.serverName << "," << req.nodeName << ","
                                              << req.userName << ", info:" << log << endl);
  return EM_I_FAILED;
}
...
EMTaskItemStatus TaskList::graceRestart(const TaskItemReq &req, string &log) {
  ...
  try {
    ...
    ret = _adminPrx->notifyServer_inner(req.application, req.serverName, req.nodeName, "kant.gracerestart", log);
    if (ret == 0) {
      return EM_I_SUCCESS;
    }
  } catch (exception &ex) {
...
EMTaskItemStatus TaskList::undeploy(const TaskItemReq &req, string &log) {
  ...
  TLOG_ERROR("TaskList::undeploy error:" << req.application << "," << req.serverName << "," << req.nodeName << ","
                                         << req.userName << ", info:" << log << endl);
...
EMTaskItemStatus TaskList::stop(const TaskItemReq &req, string &log) {
  ...
  TLOG_ERROR("TaskList::stop error:" << req.application << "," << req.serverName << "," << req.nodeName << ","
                                     << req.userName << ", info:" << log << endl);
...
EMTaskItemStatus TaskList::patch(size_t index, const TaskItemReq &req, string &log) {
  try {
    int ret = EM_KANT_UNKNOWN_ERR;

    TLOG_DEBUG("TaskList::patch:" << req.writeToJsonString() << ", "
                                  << KT_Common::tostr(req.parameters.begin(), req.parameters.end()) << endl);
    ...
    try {
      ...
      //            if (ret!=0)
      //			{
      //                log = "kantAdminRegistry batchPatch err:" + log;
    ...
    if (ret != EM_KANT_SUCCESS) {
      ...
      TLOG_ERROR("TaskList::patch error:" << req.application << "," << req.serverName << "," << req.nodeName << ","
                                          << req.userName << ", info:" << log << endl);
      return EM_I_FAILED;
    }

    // 这里做个超时保护， 否则如果kantnode状态错误， 这里一直循环调用
    time_t tNow = TNOW;
    unsigned int patchTimeout = KT_Common::strto<unsigned int>(g_pconf->get("/kant/patch/<patch_wait_timeout>", "300"));
    ...
    while (TNOW < tNow + patchTimeout) {
      ...
      if (ret != 0) {
        log = pi.sResult;
        _adminPrx->updatePatchLog_inner(req.application, req.serverName, req.nodeName, patchId, req.userName, patchType,
                                        false);
        TLOG_ERROR("getPatchPercent error, ret:" << ret << ", " << req.application << "." << req.serverName << "_"
                                                 << req.nodeName << ", percent:" << pi.iPercent
                                                 << ", log:" << pi.sResult << endl);
        return EM_I_FAILED;
      }
...
//        int ret = EM_KANT_UNKNOWN_ERR;
//
//        TLOG_DEBUG("TaskList::patch:" << KT_Common::tostr(req.parameters.begin(), req.parameters.end())
...
EMTaskItemStatus TaskList::executeSingleTask(size_t index, const TaskItemReq &req) {
  TLOG_DEBUG("TaskList::executeSingleTask: taskNo="
             << req.taskNo << ",application=" << req.application << ",serverName=" << req.serverName
             << ",nodeName=" << req.nodeName << ",setName=" << req.setName << ",command=" << req.command << endl);
  ...
  if (req.command == "stop") {
    ...
    ret = restart(req, log);
  } else if (req.command == "patch_kant") {
    ret = patch(index, req, log);
    if (ret == EM_I_SUCCESS && get("bak_flag", req.parameters) != "1") {
      //不是备机, 需要重启
      ret = restart(req, log);
    }
  } else if (req.command == "grace_patch_kant") {
    ret = patch(index, req, log);
    if (ret == EM_I_SUCCESS && get("bak_flag", req.parameters) != "1") {
      //不是备机, 需要重启
      ret = graceRestart(req, log);
    }
  } else if (req.command == "undeploy_kant") {
...
//////////////////////////////////////////////////////////////////////////////
TaskListSerial::TaskListSerial(const TaskReq &taskReq, unsigned int t) : TaskList(taskReq, t) {
  _pool.init(1);
  _pool.start();
}
...
TaskListElegant::TaskListElegant(const TaskReq &taskReq, unsigned int t) : TaskList(taskReq, t) {
  _poolMaster.init(1);
  _poolMaster.start();
}
...
void TaskListElegant::doTask() {
  ...
  for (size_t i = 0; i < len; i += eachNum) {
    if (!isSerial) {
      ...
      // 停掉流量
      int affectNum =
        DBPROXY->updateServerFlowState(tl[0].second.application, tl[0].second.serverName, nodeList, false);

      if (affectNum > 0) {
        TLOG_DEBUG("sleep " << ExecuteTask::getInstance()->getElegantWaitSecond() << "s wait for close flow..."
                            << endl);
        //sleep(75);
        KT_Common::sleep(ExecuteTask::getInstance()->getElegantWaitSecond());
      } else {
      ...
      for (size_t x = 0; x < tl.size(); ++x) {
        _pool.exec(std::bind(&TaskListElegant::doTaskParallel, this, std::placeholders::_1, std::placeholders::_2),
                   tl[x].second, tl[x].first);
      }
...
void TaskListElegant::doTaskParallel(TaskItemReq req, size_t index) {
  ...
  //do work
  TLOG_DEBUG("taskNo=" << req.taskNo << ",application=" << req.application << ",serverName=" << req.serverName
                       << ",setName=" << req.setName << ",command=" << req.command << endl);
...
//////////////////////////////////////////////////////////////////////////////
TaskListParallel::TaskListParallel(const TaskReq &taskReq, unsigned int t) : TaskList(taskReq, t) {
  //最大并行线程数
  size_t num = taskReq.taskItemReq.size() > 5 ? 5 : taskReq.taskItemReq.size();
  num = num < 1 ? 1 : num;
  TLOG_DEBUG("TaskListParallel::TaskListParallel req thread num:" << taskReq.taskItemReq.size() << ", realnum:" << num
                                                                  << endl);
  _pool.init(num);
  _pool.start();
}
...
void TaskListParallel::onExecute() {
  ...
  //    std::shared_ptr<atomic_int> TaskItemSharedState=std::make_shared<atomic_int>(TASK_ITEM_SHARED_STATED_DEFAULT);
  for (size_t i = 0; i < _taskReq.taskItemReq.size(); i++) {
    _pool.exec(std::bind(&TaskListParallel::doTask, this, std::placeholders::_1, std::placeholders::_2),
               _taskReq.taskItemReq[i], i);
  }
}
...
void TaskListParallel::doTask(TaskItemReq req, size_t index) {
  ...
  //do work
  TLOG_DEBUG("TaskListParallel::executeTask: taskNo=" << req.taskNo << ",application=" << req.application
                                                      << ",serverName=" << req.serverName << ",setName=" << req.setName
                                                      << ",command=" << req.command << endl);
...
ExecuteTask::~ExecuteTask() { terminate(); }

void ExecuteTask::terminate() {
  _terminate = true;
  KT_LockT<KT_ThreadLock> lock(*this);
  notifyAll();
}
...
int ExecuteTask::addTaskReq(const TaskReq &taskReq) {
  TLOG_DEBUG("ExecuteTask::addTaskReq"
             << ", taskNo=" << taskReq.taskNo << ", size=" << taskReq.taskItemReq.size()
             << ", serial=" << taskReq.serial << ", isElegant:" << taskReq.isElegant << ", eachNum:" << taskReq.eachNum
...
void ExecuteTask::checkTaskRspStatus(TaskRsp &taskRsp) {
  ...
  if (not_start == taskRsp.taskItemRsp.size())
    taskRsp.status = EM_T_NOT_START;
  else if (prepare == taskRsp.taskItemRsp.size())
    taskRsp.status = EM_T_PREPARE;
  else if (running == taskRsp.taskItemRsp.size())
    taskRsp.status = EM_T_RUNNING;
  else if (success == taskRsp.taskItemRsp.size())
    taskRsp.status = EM_T_SUCCESS;
  else if (failed == taskRsp.taskItemRsp.size())
    taskRsp.status = EM_T_FAILED;
  else if (cancel == taskRsp.taskItemRsp.size())
    taskRsp.status = EM_T_CANCEL;
  else
    taskRsp.status = EM_T_PARIAL;
}

void ExecuteTask::init(KT_Config *conf) {
  _elegantWaitSecond =
    KT_Common::strto<unsigned int>(conf->get("/kant/application/server/<elegant_wait_second>", "75"));
