...
AdapterProxy::AdapterProxy(ObjectProxy* pObjectProxy, const EndpointInfo& ep, Communicator* pCom)
  : _communicator(pCom),
    ...
    _id((++_idGen)) {
  _timeoutQueue.reset(new KT_TimeoutQueueNew<ReqMessage*>());
...
#if KANT_SSL
  if (ep.isSsl()) {
    _trans.reset(new KT_SSLTransceiver(pObjectProxy->getCommunicatorEpoll()->getEpoller(), ep.getEndpoint()));
  } else if (ep.isTcp()) {
    _trans.reset(new KT_TCPTransceiver(pObjectProxy->getCommunicatorEpoll()->getEpoller(), ep.getEndpoint()));
  } else {
    _trans.reset(new KT_UDPTransceiver(pObjectProxy->getCommunicatorEpoll()->getEpoller(), ep.getEndpoint()));
  }
#else
  if (ep.isUdp()) {
    _trans.reset(new KT_UDPTransceiver(pObjectProxy->getCommunicatorEpoll()->getEpoller(), ep.getEndpoint()));
  ...
  _trans->initializeClient(
    std::bind(&AdapterProxy::onCreateCallback, this, std::placeholders::_1),
    std::bind(&AdapterProxy::onCloseCallback, this, std::placeholders::_1, std::placeholders::_2,
              std::placeholders::_3),
  ...
  _trans->setClientAuthCallback(
    std::bind(&AdapterProxy::onSendAuthCallback, this, std::placeholders::_1),
...
shared_ptr<KT_ProxyInfo> AdapterProxy::onCreateCallback(KT_Transceiver* trans) {
  // LOG_CONSOLE_DEBUG << "fd:" << trans->fd() << ", " << trans << endl;

  _objectProxy->getCommunicatorEpoll()->addFd(this);

  trans->setConnTimeout(_objectProxy->getRootServantProxy()->kant_connect_timeout());

  if (_objectProxy->getRootServantProxy()->getProxyInfo()) {
    return KT_ProxyInfo::createProxyInfo(*_objectProxy->getRootServantProxy()->getProxyInfo());
  }
  return NULL;
}

std::shared_ptr<KT_OpenSSL> AdapterProxy::onOpensslCallback(KT_Transceiver* trans) {
  //	LOG_CONSOLE_DEBUG << "fd:" << trans->fd() << ", " << trans << endl;
  return _objectProxy->getCommunicatorEpoll()->getCommunicator()->newClientSSL(_objectProxy->name());
}

void AdapterProxy::onCloseCallback(KT_Transceiver* trans, KT_Transceiver::CloseReason reason, const string& err) {
  if (_objectProxy->getRootServantProxy()->kant_get_push_callback()) {
    _objectProxy->getRootServantProxy()->kant_get_push_callback()->onClose();
  }
  ...
  if (second > 0) {
    ...
    TLOGERROR("[trans close:" << _objectProxy->name() << "," << trans->getConnectEndpoint().toString()
                              << ", reconnect:" << second << "]" << endl);
  }
}

void AdapterProxy::onConnectCallback(KT_Transceiver* trans) {
  // LOG_CONSOLE_DEBUG << "fd:" << trans->fd() << ", " << trans << endl;
  addConnExc(false);

  if (_objectProxy->getRootServantProxy()->kant_get_push_callback()) {
    _objectProxy->getRootServantProxy()->kant_get_push_callback()->onConnect(trans->getConnectEndpoint());
  }

  _objectProxy->onConnect(this);
}

void AdapterProxy::onRequestCallback(KT_Transceiver* trans) {
  //	LOG_CONSOLE_DEBUG  << "fd:" << trans->fd() << ", " << trans << endl;
  doInvoke();
}

KT_NetWorkBuffer::PACKET_TYPE AdapterProxy::onParserCallback(KT_NetWorkBuffer& buff, KT_Transceiver* trans) {
  //	LOG_CONSOLE_DEBUG  << "fd:" << trans->fd() << ", " << trans<< endl;
  try {
    shared_ptr<ResponsePacket> rsp = std::make_shared<ResponsePacket>();

    KT_NetWorkBuffer::PACKET_TYPE ret =
      _objectProxy->getRootServantProxy()->kant_get_protocol().responseFunc(buff, *rsp.get());
...
void AdapterProxy::onCompletePackage(KT_Transceiver* trans) {
  //	LOG_CONSOLE_DEBUG  << "fd:" << trans->fd() << ", " << trans << endl;
  if (_objectProxy->getRootServantProxy()->kant_connection_serial() > 0) {
...
shared_ptr<KT_NetWorkBuffer::Buffer> AdapterProxy::onSendAuthCallback(KT_Transceiver* trans) {
  //	LOG_CONSOLE_DEBUG  << "fd:" << trans->fd() << ", " << trans << endl;
  ...
  info.sAccessKey =
    _objectProxy->getCommunicatorEpoll()->getCommunicator()->getServantProperty(_objectProxy->name(), "accesskey");
  info.sSecretKey =
    _objectProxy->getCommunicatorEpoll()->getCommunicator()->getServantProperty(_objectProxy->name(), "secretkey");

  //    std::string out = kant::defaultCreateAuthReq(info);

  const int kAuthType = 0x40;
  RequestPacket request;
  request.sFuncName = "InnerAuthServer";
  request.sServantName = "authServant";
  request.iVersion = KANTVERSION;
  request.iRequestId = 1;
  request.cPacketType = KANTNORMAL;
  request.iMessageType = kAuthType;
  request.sBuffer = kant::defaultCreateAuthReq(info);  //.assign(out.begin(), out.end());

  return _objectProxy->getRootServantProxy()->kant_get_protocol().requestFunc(request, trans);
}

KT_NetWorkBuffer::PACKET_TYPE AdapterProxy::onVerifyAuthCallback(KT_NetWorkBuffer& buff, KT_Transceiver* trans) {
  shared_ptr<ResponsePacket> rsp = std::make_shared<ResponsePacket>();
  ...
  if (ret == KT_NetWorkBuffer::PACKET_FULL) {
    ...
    if (KT_Port::strncasecmp(rsp->sBuffer.data(), "AUTH_SUCC", rsp->sBuffer.size()) == 0) {
...
void AdapterProxy::initStatHead() {
  ...
  if (!ClientConfig::SetDivision.empty() && StatReport::divison2SetInfo(ClientConfig::SetDivision, vtSetInfo)) {
    //主调(client)启用set
    _statHead.masterName = StatReport::trimAndLimitStr(
      ClientConfig::ModuleName + "." + vtSetInfo[0] + vtSetInfo[1] + vtSetInfo[2] + "@" + ClientConfig::KantVersion,
      StatReport::MAX_MASTER_NAME_LEN);
  } else {
    _statHead.masterName = StatReport::trimAndLimitStr(ClientConfig::ModuleName + "@" + ClientConfig::KantVersion,
                                                       StatReport::MAX_MASTER_NAME_LEN);
  }
  ...
  if (!sSlaveSet.empty() && StatReport::divison2SetInfo(sSlaveSet, vtSetInfo))  //被调启用set
  {
    ...
    _statHead.slaveName = StatReport::trimAndLimitStr(sSlaveName + "." + vtSetInfo[0] + vtSetInfo[1] + vtSetInfo[2],
                                                      StatReport::MAX_MASTER_NAME_LEN);
  } else {
...
void AdapterProxy::onConnect() { _objectProxy->onConnect(this); }

int AdapterProxy::invoke_connection_serial(ReqMessage* msg) {
  assert(msg->eType != ReqMessage::ONE_WAY);

  msg->sReqData = _objectProxy->getRootServantProxy()->kant_get_protocol().requestFunc(msg->request, _trans.get());
  ...
  if (!_requestMsg && _timeoutQueue->sendListEmpty()) {
    ...
    if (ret == KT_Transceiver::eRetOk || ret == KT_Transceiver::eRetFull) {
      TLOGKANT("[AdapterProxy::invoke_connection_serial push (send) obj: "
               << _objectProxy->name() << ", desc:" << _trans->getConnectionString()
               << ", id: " << msg->request.iRequestId << endl);

      _requestMsg = msg;

      bool bFlag = _timeoutQueue->push(msg, msg->request.iRequestId, msg->request.iTimeout + msg->iBeginTime);
      if (!bFlag) {
        TLOGERROR("[AdapterProxy::invoke_connection_serial fail1 : insert timeout queue fail,queue size:"
                  << _timeoutQueue->size() << ",id: " << msg->request.iRequestId << "," << _objectProxy->name() << ", "
      ...
      return 0;
    } else if (ret == KT_Transceiver::eRetError) {
      //发送出错了
      _requestMsg = NULL;
  ...
  //数据没有发送
  TLOGKANT("[AdapterProxy::invoke_connection_serial push (no send) "
           << _objectProxy->name() << ", " << _trans->getConnectionString() << ",id " << msg->request.iRequestId << ", "
           << _requestMsg << endl);

  //数据没有发送
  bool bFlag = _timeoutQueue->push(msg, msg->request.iRequestId, msg->request.iTimeout + msg->iBeginTime, false);
  if (!bFlag) {
    TLOGERROR("[AdapterProxy::invoke_connection_serial fail2 : insert timeout queue fail,queue size:"
              << _timeoutQueue->size() << ", id: " << msg->request.iRequestId << ", " << _objectProxy->name() << ", "
              << _trans->getConnectionString() << "]" << endl);
    msg->eStatus = ReqMessage::REQ_EXC;
...
int AdapterProxy::invoke_connection_parallel(ReqMessage* msg) {
  ...
  if (_timeoutQueue->sendListEmpty()) {
    ...
    if (ret == KT_Transceiver::eRetOk || ret == KT_Transceiver::eRetFull) {
      TLOGKANT("[AdapterProxy::invoke_connection_parallel push (send) obj: "
               << _objectProxy->name() << ", desc:" << _trans->getConnectionString()
               << ", id: " << msg->request.iRequestId << endl);
      ...
      bool bFlag = _timeoutQueue->push(msg, msg->request.iRequestId, msg->request.iTimeout + msg->iBeginTime);
      if (!bFlag) {
        TLOGERROR("[AdapterProxy::invoke_connection_parallel fail1 : insert timeout queue fail,queue size:"
                  << _timeoutQueue->size() << ",id: " << msg->request.iRequestId << "," << _objectProxy->name() << ", "
                  << _trans->getConnectionString() << "]" << endl);
        msg->eStatus = ReqMessage::REQ_EXC;
      ...
      TLOGKANT("[AdapterProxy::invoke_connection_parallel send request failed,queue size:"
               << _timeoutQueue->size() << ",id: " << msg->request.iRequestId << "," << _objectProxy->name() << ", "
               << _trans->getConnectionString() << "]" << endl);
  ...
  //没有发送数据
  TLOGKANT("[AdapterProxy::invoke_connection_parallel push (no send) "
           << _objectProxy->name() << ", " << _trans->getConnectionString() << ",id " << msg->request.iRequestId
           << endl);
  ...
  bool bFlag = _timeoutQueue->push(msg, msg->request.iRequestId, msg->request.iTimeout + msg->iBeginTime, false);
  if (!bFlag) {
    TLOGERROR("[AdapterProxy::invoke_connection_parallel fail2 : insert timeout queue fail,queue size:"
              << _timeoutQueue->size() << ", id: " << msg->request.iRequestId << ", " << _objectProxy->name() << ", "
              << _trans->getConnectionString() << "]" << endl);
    msg->eStatus = ReqMessage::REQ_EXC;
...
int AdapterProxy::invoke(ReqMessage* msg) {
  ...
  TLOGKANT("[AdapterProxy::invoke " << _objectProxy->name() << ", " << _trans->getConnectionString() << "]" << endl);

  //未发链表有长度限制
  if (_timeoutQueue->getSendListSize() > _noSendQueueLimit) {
    TLOGERROR("[AdapterProxy::invoke fail,ReqInfoQueue.size(" << _timeoutQueue->getSendListSize() << ") > "
                                                              << _noSendQueueLimit << "," << _objectProxy->name() << ","
                                                              << _trans->getConnectionString() << "]" << endl);

    msg->eStatus = ReqMessage::REQ_EXC;

    msg->response->iRet = kant::KANTSENDREQUESTERR;
    finishInvoke(msg);
    return 0;
  }
...
void AdapterProxy::doInvoke_serial() {
  if (_requestMsg != NULL || _timeoutQueue->sendListEmpty()) {
    ...
    TLOGKANT("[AdapterProxy::doInvoke_serial not send obj:"
             << _objectProxy->name() << ",desc:" << _trans->getConnectionString()
             << ", send size:" << _timeoutQueue->getSendListSize() << ", " << _requestMsg << endl);
    return;
  }
  ...
  if (iRet == KT_Transceiver::eRetError) {
    ...
    msg->response->iRet = KANTSENDREQUESTERR;

    TLOGKANT("[AdapterProxy::doInvoke_serial sendRequest failed, obj:"
             << _objectProxy->name() << ",desc:" << _trans->getConnectionString() << ",id:" << msg->request.iRequestId
             << ", ret:" << iRet << endl);

    finishInvoke(msg);

  } else if (iRet == KT_Transceiver::eRetOk || iRet == KT_Transceiver::eRetFull) {
    _requestMsg = msg;
...
void AdapterProxy::doInvoke_parallel() {
  while (!_timeoutQueue->sendListEmpty()) {
    ...
    //发送失败 or 没有发送
    if (iRet == KT_Transceiver::eRetError) {
      TLOGKANT("[AdapterProxy::doInvoke_parallel sendRequest failed, obj:"
               << _objectProxy->name() << ",desc:" << _trans->getConnectionString() << ",id:" << msg->request.iRequestId
               << ", ret:" << iRet << endl);
      return;
    }
    ...
    if (iRet == KT_Transceiver::eRetNotSend) {
      TLOGKANT("[AdapterProxy::doInvoke_parallel sendRequest not send, obj:"
               << _objectProxy->name() << ",desc:" << _trans->getConnectionString() << ",id:" << msg->request.iRequestId
               << ", ret:" << iRet << endl);
      return;
    }
    ...
    //发送buffer已经满了 要返回
    if (iRet == KT_Transceiver::eRetFull) {
      return;
    }
  }
}
...
void AdapterProxy::finishInvoke(bool bFail) {
  TLOGKANT("[AdapterProxy::finishInvoke(bool), " << _objectProxy->name() << ", " << _trans->getConnectionString() << ","
                                                 << bFail << "]" << endl);

  time_t now = TNOW;

  const CheckTimeoutInfo& info = _objectProxy->getRootServantProxy()->kant_check_timeout_info();
  ...
  if (bFail) {
    ...
    if (_frequenceFailInvoke >= info.frequenceFailInvoke && now >= _frequenceFailTime) {
      ...
      TLOGERROR("[AdapterProxy::finishInvoke(bool) objname:"
                << _objectProxy->name() << ",desc:" << _trans->getConnectionString()
                << ",disable frequenceFail,freqtimeout:" << _frequenceFailInvoke << ",timeout:" << _timeoutInvoke
  ...
  if (now > _nextFinishInvokeTime) {
    ...
    if (bFail && _timeoutInvoke >= info.minTimeoutInvoke && _timeoutInvoke >= info.radio * _totalInvoke) {
      ...
      TLOGERROR("[AdapterProxy::finishInvoke(bool), "
                << _objectProxy->name() << "," << _trans->getConnectionString() << ",disable radioFail,freqtimeout:"
                << _frequenceFailInvoke << ",timeout:" << _timeoutInvoke << ",total:" << _totalInvoke << "] " << endl);
    } else {
      //每一分钟清空一次
      _totalInvoke = 0;
...
void AdapterProxy::resetRetryTime(bool next) {
  if (next) {
    _nextRetryTime = TNOW + _objectProxy->getRootServantProxy()->kant_check_timeout_info().tryTimeInterval;
  } else {
    _nextRetryTime = TNOW;
  }
}
...
bool AdapterProxy::checkActive(bool connecting) {
  ...
  TLOGKANT(
    "[AdapterProxy::checkActive," << _objectProxy->name() << "," << _trans->getConnectionString() << ","
                                  << (_activeStatus ? "active" : "inactive") << (connecting ? ", connecting" : "")
                                  << ", freqtimeout:" << _frequenceFailInvoke << ", timeout:" << _timeoutInvoke
                                  << ", connExcCnt:" << _connExcCnt << ", total:" << _totalInvoke << "]" << endl);

  //失效且没有到下次重试时间, 直接返回不可用
  if ((!_activeStatus) && (now < _nextRetryTime)) {
    TLOGKANT("[AdapterProxy::checkActive,not reach retry time ," << _objectProxy->name() << ","
                                                                 << _trans->getConnectionString() << endl);
    return false;
  }
  ...
  if (!_trans->isValid()) {
    try {
      ...
      TLOGERROR("[AdapterProxy::checkActive connect obj:"
                << _objectProxy->name() << ",desc:" << _trans->getConnectionString() << ", ex:" << ex.what() << endl);
    }
  }
...
void AdapterProxy::setInactive() {
  ...
  TLOGKANT("[AdapterProxy::setInactive, " << _objectProxy->name() << ", " << _trans->getConnectionString()
                                          << ", inactive]" << endl);
}

void AdapterProxy::finishInvoke_serial(shared_ptr<ResponsePacket>& rsp) {
  TLOGKANT("[AdapterProxy::finishInvoke_serial, " << _objectProxy->name() << ", " << _trans->getConnectionString()
                                                  << ", id:" << rsp->iRequestId << "]" << endl);

  if (!_requestMsg) {
    if (_timeoutLogFlag) {
      TLOGERROR("[AdapterProxy::finishInvoke_serial," << _objectProxy->name()
                                                      << ", get req-ptr NULL,may be timeout,id:" << rsp->iRequestId
                                                      << ",desc:" << _trans->getConnectionString() << "]" << endl);
    }
    return;
  }
...
void AdapterProxy::finishInvoke_parallel(shared_ptr<ResponsePacket>& rsp) {
  TLOGKANT("[AdapterProxy::finishInvoke_parallel, " << _objectProxy->name() << ", " << _trans->getConnectionString()
                                                    << ", id:" << rsp->iRequestId << "]" << endl);
  ...
  if (rsp->iRequestId == 0) {
    ...
    if (!_objectProxy->getRootServantProxy()->kant_get_push_callback()) {
      TLOGERROR("[AdapterProxy::finishInvoke(BasePacket)， request id is 0, pushcallback is null, "
                << _objectProxy->name() << ", " << _trans->getConnectionString() << "]" << endl);
      throw KantDecodeException("request id is 0, pushcallback is null, obj: " + _objectProxy->name() +
    ...
    msg->adapter = this;
    msg->callback = _objectProxy->getRootServantProxy()->kant_get_push_callback();
  } else {
    //这里的队列中的发送链表中的数据可能已经在timeout的时候删除了，因此可能会core，在erase中要加判断
    //获取请求信息
    ...
    if (!retErase) {
      if (_timeoutLogFlag) {
        TLOGERROR("[AdapterProxy::finishInvoke_parallel," << _objectProxy->name()
                                                          << ",get req-ptr NULL,may be timeout,id:" << rsp->iRequestId
                                                          << ",desc:" << _trans->getConnectionString() << "]" << endl);
      }
      return;
...
void AdapterProxy::finishInvoke(ReqMessage* msg) {
  ...
  TLOGKANT("[AdapterProxy::finishInvokeMsg " << _objectProxy->name() << ", " << _trans->getConnectionString()
  ...
  if (msg->eType == ReqMessage::ONE_WAY) {
    TLOGKANT("[AdapterProxy::finishInvokeMsg " << _objectProxy->name() << ", " << _trans->getConnectionString()
                                               << " ,id:" << msg->response->iRequestId << " ,one way call]" << endl);
    delete msg;
    msg = NULL;
    return;
  }
  ...
  if (msg->eStatus != ReqMessage::REQ_EXC && !msg->bPush) {
    finishInvoke(msg->response->iRet != KANTSERVERSUCCESS);
  }
  ...
  if (msg->eType == ReqMessage::ASYNC_CALL) {
    if (!msg->sched) {
      ...
      if (ptr) {
        ...
        TLOGERROR("[AdapterProxy::finishInvoke(ReqMessage) coro parallel callback error,obj:"
                  << _objectProxy->name() << ",endpoint:" << _trans->getConnectionString()
                  << " ,id:" << msg->response->iRequestId << "]" << endl);
        delete msg;
        msg = NULL;
      }
...
void AdapterProxy::doTimeout() {
  ...
  while (_timeoutQueue->timeout(msg)) {
    TLOGKANT("[AdapterProxy::doTimeout, " << _objectProxy->name() << ", " << _trans->getConnectionString() << ", id:"
                                          << msg->request.iRequestId << ", status:" << msg->eStatus << "]" << endl);
    ...
    if (msg->eType == ReqMessage::ASYNC_CALL) {
      ...
      msg->response->iRet = (_connExcCnt > 0 ? KANTPROXYCONNECTERR : KANTASYNCCALLTIMEOUT);
    }
...
void AdapterProxy::doKeepAlive() {
  ...
  TLOGKANT("[AdapterProxy::doKeepAlive, " << _objectProxy->name() << ", " << _trans->getConnectionString() << "]"
                                          << endl);
  ...
  msg->request.cPacketType = KANTONEWAY;
  msg->request.sFuncName = "kant_ping";
  msg->request.sServantName = _objectProxy->name();

  msg->request.iTimeout = ServantProxy::DEFAULT_ASYNCTIMEOUT;

  msg->proxy = _objectProxy->getServantProxy();
  msg->response->iRet = KANTSERVERUNKNOWNERR;
...
//     {
//         TLOGKANT("tracer info is null, just return" << endl);
//         return;
//     }
...
//     SET_MSG_TYPE(msg->request.iMessageType, kant::KANTMESSAGETYPETRACK);
// }
...
void AdapterProxy::stat(ReqMessage* msg) {
  ...
  //包体信息.
  if (msg->eStatus == ReqMessage::REQ_RSP && KANTSERVERSUCCESS == msg->response->iRet) {
    body.count = 1;
...
void AdapterProxy::addConnExc(bool bExc) {
  if (bExc) {
    if (!_connExc && ++_connExcCnt >= _objectProxy->getRootServantProxy()->kant_check_timeout_info().maxConnectExc) {
