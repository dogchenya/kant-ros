#ifndef _KANT_PARSE_H_
#define _KANT_PARSE_H_
...
class KantParse {
 public:
  ...
  void setKeyStruct(const StructPtr &key) { _key = key; }

  StructPtr getKeyStruct() { return _key; }
  ...
  /**
     ...
     * 设置include的kant文件全部从当前文件搜寻 
     */
  void setUseCurrentPath(bool bEnable) { _bUseCurrentPath = bEnable; }

  /**
     * 查找kant文件时,设置include路径
     */
  void addIncludePath(const string &include) {
    vector<string> v = kant::KT_Common::sepstr<string>(include, "; ,", false);
...
typedef std::shared_ptr<KantParse> KantParsePtr;

extern KantParsePtr g_parse;
