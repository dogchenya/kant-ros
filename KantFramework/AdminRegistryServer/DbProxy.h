 ...
 * 主控获取node信息异常
 */
struct KantNodeNotRegistryException : public KantException {
  KantNodeNotRegistryException(const string &buffer) : KantException(buffer){};
  KantNodeNotRegistryException(const string &buffer, int err) : KantException(buffer, err){};
  ~KantNodeNotRegistryException() throw(){};
};

/**
 *  数据库操作类
 */
class DbProxy : public kant::KT_Singleton<DbProxy> {
 public:
  /**
     * 构造函数
     */
  ...
  /**
     ...
     */
  int init(KT_Config *pconf);

  /**
     * 获取框架服务
     */
  int getFramework(vector<kant::FrameworkServer> &servers);
  /**
     * 获取特定node id的对象代理
  ...
  /**
     ...
     */
  int getTaskHistory(const string &application, const string &serverName, const string &command,
  ...
  /**
     ...
     * @return int 
     */
  int undeploy(const string &application, const string &serverName, const string &nodeName, const string &user,
               string &log);
  ...
  /**
     ...
     * @return server信息列表
     */
  vector<ServerDescriptor> getServers(const string &app, const string &serverName, const string &nodeName,
                                      bool withDnsServer = false);
  ...
  /**
     ...
     * @return int 
     */
  int updatePatchByPatchId(const string &application, const string &serverName, const string &nodeName,
 ...
 public:
  /**
     ...
     * @return server信息列表
     */
  int updateServerState(const string &app, const string &serverName, const string &nodeName, const string &stateFields,
                        kant::ServerState state, int processId = -1);
  ...
  /**
     ...
     * @param user     : 发布者
     */
  int setPatchInfo(const string &app, const string &serverName, const string &nodeName, const string &version,
                   const string &user);
  ...
  /**
     * 只保留必要的数据
     */
  void doReserveDb(const string path, KT_Config *pconf);

  //    int getServerInfo(const kant::srvRequestInfo & info,vector<kant::serverInfo>& vServerInfo);
 protected:
  //mysql连接对象
  static vector<KT_Mysql *> _mysqlReg;
  static vector<KT_ThreadMutex *> _mysqlLocks;

  //node节点代理列表
  static map<string, NodePrx> _mapNodePrxCache;
  static KT_ThreadLock _NodePrxLock;

  //匹配分组信息
  static vector<map<string, string>> _serverGroupRule;

  //用于初始化保护
  static KT_ThreadLock _mutex;
