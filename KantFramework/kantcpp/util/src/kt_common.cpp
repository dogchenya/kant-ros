...
const float KT_Common::_EPSILON_FLOAT = 0.000001f;
const double KT_Common::_EPSILON_DOUBLE = 0.000001;

void KT_Common::sleep(uint32_t sec) { std::this_thread::sleep_for(std::chrono::seconds(sec)); }

void KT_Common::msleep(uint32_t ms) { std::this_thread::sleep_for(std::chrono::milliseconds(ms)); }

bool KT_Common::equal(double x, double y, double epsilon) { return fabs(x - y) < epsilon; }

bool KT_Common::equal(double x, double y, float epsilon) { return equal(x, y, (double)epsilon); }

bool KT_Common::equal(float x, float y, float epsilon) { return fabsf(x - y) < epsilon; }

bool KT_Common::equal(float x, float y, double epsilon) { return equal(x, y, float(epsilon)); }

bool KT_Common::equal(const vector<double> &vx, const vector<double> &vy, double epsilon) { MATCH_DOUBLE }

bool KT_Common::equal(const vector<double> &vx, const vector<double> &vy, float epsilon) {
  return equal(vx, vy, double(epsilon));
}

bool KT_Common::equal(const vector<float> &vx, const vector<float> &vy, float epsilon) { MATCH_DOUBLE }

bool KT_Common::equal(const vector<float> &vx, const vector<float> &vy, double epsilon) {
  return equal(vx, vy, float(epsilon));
}

bool KT_Common::equal(const set<double> &vx, const set<double> &vy, double epsilon) { MATCH_DOUBLE }

bool KT_Common::equal(const set<double> &vx, const set<double> &vy, float epsilon) {
  return equal(vx, vy, double(epsilon));
}

bool KT_Common::equal(const set<float> &vx, const set<float> &vy, float epsilon) {
  return equal(vx, vy, double(epsilon));
}

bool KT_Common::equal(const set<float> &vx, const set<float> &vy, double epsilon) { MATCH_DOUBLE }

bool KT_Common::equal(const unordered_set<double> &vx, const unordered_set<double> &vy, double epsilon) { MATCH_DOUBLE }

bool KT_Common::equal(const unordered_set<double> &vx, const unordered_set<double> &vy, float epsilon) {
  return equal(vx, vy, double(epsilon));
}

bool KT_Common::equal(const unordered_set<float> &vx, const unordered_set<float> &vy, float epsilon) {
  return equal(vx, vy, double(epsilon));
}

bool KT_Common::equal(const unordered_set<float> &vx, const unordered_set<float> &vy, double epsilon) { MATCH_DOUBLE }
...
string KT_Common::trim(const string &sStr, const string &s, bool bChar) {
  if (sStr.empty()) {
    return sStr;
  }
...
string KT_Common::trimleft(const string &sStr, const string &s, bool bChar) {
  if (sStr.empty()) {
    return sStr;
  }
...
string KT_Common::trimright(const string &sStr, const string &s, bool bChar) {
  if (sStr.empty()) {
    return sStr;
  }
...
string KT_Common::lower(const string &s) {
  string sString = s;
...
string KT_Common::upper(const string &s) {
  string sString = s;
...
bool KT_Common::isdigit(const string &sInput) {
...
int KT_Common::str2tm(const string &sString, const string &sFormat, struct tm &stTm) {
...
int KT_Common::strgmt2tm(const string &sString, struct tm &stTm) {
  return str2tm(sString, "%a, %d %b %Y %H:%M:%S GMT", stTm);
}

string KT_Common::tm2str(const struct tm &stTm, const string &sFormat) {
...
string KT_Common::tm2str(const time_t &t, const string &sFormat) {
  struct tm tt;
  tm2time(t, tt);
...
void KT_Common::tm2tm(const time_t &t, struct tm &tt) { tm2time(t, tt); }

string KT_Common::now2str(const string &sFormat) {
  time_t t = time(NULL);
  return tm2str(t, sFormat.c_str());
}

// string KT_Common::now2msstr()
//{
//    return ms2str(now2ms());
//}

string KT_Common::ms2str(int64_t ms) {
  time_t t = ms / 1000;

  auto duration_in_ms = ms % 1000;
  tm tt;

  KT_Port::localtime_r(&t, &tt);
...
string KT_Common::now2GMTstr() {
  time_t t = time(NULL);
  return tm2GMTstr(t);
}

string KT_Common::tm2GMTstr(const time_t &t) {
  struct tm tt;

  KT_Port::gmtime_r(&t, &tt);
  return tm2str(tt, "%a, %d %b %Y %H:%M:%S GMT");
}

string KT_Common::tm2GMTstr(const struct tm &stTm) { return tm2str(stTm, "%a, %d %b %Y %H:%M:%S GMT"); }

string KT_Common::nowdate2str() { return now2str("%Y%m%d"); }

string KT_Common::nowtime2str() { return now2str("%H%M%S"); }

int64_t KT_Common::now2ms() {
  struct timeval tv;
...
string KT_Common::bin2str(const void *buf, size_t len, const string &sSep, size_t lines) {
  if (buf == NULL || len <= 0) {
    return "";
  }
...
string KT_Common::bin2str(const string &sBinData, const string &sSep, size_t lines) {
  return bin2str((const void *)sBinData.data(), sBinData.length(), sSep, lines);
}

int KT_Common::str2bin(const char *psAsciiData, unsigned char *sBinData, int iBinSize) {
...
string KT_Common::str2bin(const string &sString, const string &sSep, size_t lines) {
...
char KT_Common::x2c(const string &sWhat) {
  register char digit;
...
string KT_Common::replace(const string &sString, const string &sSrc, const string &sDest) {
  if (sSrc.empty()) {
    return sString;
  }
...
string KT_Common::replace(const string &sString, const map<string, string> &mSrcDest) {
  if (sString.empty()) {
    return sString;
  }
...
bool KT_Common::matchPeriod(const string &s, const string &pat) {
  if (s.empty()) {
    return false;
  }
...
bool KT_Common::matchPeriod(const string &s, const vector<string> &pat) {
  for (size_t i = 0; i < pat.size(); i++) {
    if (KT_Common::matchPeriod(s, pat[i])) {
      return true;
    }
  }
  return false;
...
bool KT_Common::isPrimeNumber(size_t n) {
...
size_t KT_Common::toSize(const string &s, size_t iDefaultSize) {
  if (s.empty()) {
    return iDefaultSize;
  }
  ...
  if (c != 'K' && c != 'M' && c != 'G' && KT_Common::trim(s) == KT_Common::tostr(KT_Common::strto<size_t>(s))) {
    ...
    float n = KT_Common::strto<float>(s.substr(0, s.length() - 1));

    if (KT_Common::trim(s.substr(0, s.length() - 1)) != KT_Common::tostr(n)) {
      return iDefaultSize;
    }
...
void KT_Common::getRandomHexChars(char *p, unsigned int len) {
...
string KT_Common::nextDate(const string &sDate) {
  time_t tomorrow = KT_Common::str2time(sDate + "000000", "%Y%m%d%H%M%S") + 60 * 60 * 24;

  return KT_Common::tm2str(tomorrow, "%Y%m%d");
}

string KT_Common::prevDate(const string &sDate) {
  time_t tomorrow = KT_Common::str2time(sDate + "000000", "%Y%m%d%H%M%S") - 60 * 60 * 24;

  return KT_Common::tm2str(tomorrow, "%Y%m%d");
}

int KT_Common::nextDate(int iDate) { return KT_Common::strto<int>(nextDate(KT_Common::tostr(iDate))); }

int KT_Common::prevDate(int iDate) { return KT_Common::strto<int>(prevDate(KT_Common::tostr(iDate))); }

string KT_Common::nextMonth(const string &sMonth) {
  if (sMonth.length() != 6) {
    return "197001";
  }
  int year = KT_Common::strto<int>(sMonth.substr(0, 4));
  int month = KT_Common::strto<int>(sMonth.substr(4, 2));
  ...
  return KT_Common::tostr(yearNew) + szMonth;
}

string KT_Common::prevMonth(const string &sMonth) {
  if (sMonth.length() != 6) {
    return "197001";
  }
  int year = KT_Common::strto<int>(sMonth.substr(0, 4));
...
string KT_Common::nextYear(const string &sYear) {
  if (sYear.length() != 4) {
    return "1970";
  }
  int year = KT_Common::strto<int>(sYear.substr(0, 4));
  int yearNew = year + 1;

  yearNew = (yearNew >= 1970 && yearNew <= 9999) ? yearNew : 1970;

  return KT_Common::tostr(yearNew);
}

string KT_Common::prevYear(const string &sYear) {
  if (sYear.length() != 4) {
    return "1970";
  }
  int year = KT_Common::strto<int>(sYear.substr(0, 4));
...
int KT_Common::secondsToDateInt(time_t seconds) {
  struct tm tt;

  KT_Common::tm2tm(seconds, tt);
...
string KT_Common::secondsToMondayString(time_t seconds) {
  ...
  return KT_Common::tm2str(seconds, "%Y%m%d");
}

string KT_Common::secondsToMonthString(time_t seconds) { return KT_Common::tm2str(seconds, "%Y-%m"); }

string KT_Common::msToTimeString(int64_t ms) {
  int64_t sec = ms / 1000;
  ...
  return KT_Common::tm2str(sec, "%Y%m%d-%H%M%S-") + string(szMS);
}

int64_t KT_Common::timeStringToMs(const string &timeStr) {
  vector<string> vt = KT_Common::sepstr<string>(timeStr, "-");
  if (vt.size() != 3) {
    return 0;
  }
  ...
  int64_t lsec = KT_Common::str2time(date + time);
  int64_t lms = KT_Common::strto<int64_t>(ms);

  return lsec * 1000 + lms;
}

bool KT_Common::getSectionFromMs(int64_t ms, string &date, string &time, string &mstick) {
...
string KT_Common::getDateFromMs(int64_t ms) {
  string date;
  string time;
  string mstick;
...
string KT_Common::getTimeFromMs(int64_t ms) {
  string date;
  string time;
  string mstick;
  if (!getSectionFromMs(ms, date, time, mstick)) {
...
int KT_Common::nowDaySeconds() { return KT_Common::msToNowSeconds(KT_Common::now2ms()); }
...
int64_t KT_Common::us() {
  struct timeval tv;
  KT_Common::gettimeofday(tv);
  return tv.tv_sec * (int64_t)1000000 + tv.tv_usec;
}

int64_t KT_Common::dateToMs(const string &sDate) {
  int64_t now;
...
int64_t KT_Common::dateToSecond(const string &sDate) {
  int64_t now;
...
int KT_Common::dateTo(const string &sDate, const string &sPeriod) {
  if (sPeriod == "W") {
...
int KT_Common::dateToWeekday(const string &sDate) {
  struct tm date;
  KT_Common::str2tm(sDate, "%Y%m%d", date);

  return date.tm_wday;
}

int KT_Common::dateToWeek(const string &sDate) {
  struct tm date;
  KT_Common::str2tm(sDate, "%Y%m%d", date);

  return KT_Common::strto<int>(KT_Common::tm2str(date, "%W"));
}

int KT_Common::dateToMonth(const string &sDate) { return KT_Common::strto<int>(sDate.substr(4, 2)); }

int KT_Common::dateToSeason(const string &sDate) {
...
int KT_Common::dateToHalfYear(const string &sDate) {
  string month = sDate.substr(4, 2);
...
int KT_Common::dateToYear(const string &sDate) { return KT_Common::strto<int>(sDate.substr(0, 4)); }

int64_t KT_Common::timeToDaySec(int64_t hour, int64_t min) { return hour * 3600 + min * 60; }

int64_t KT_Common::getNextAbsClockMs(int64_t clockSec) {
  int64_t now = KT_Common::now2ms();
...
int KT_Common::nextDate(int iDate, int offset) {
  if (offset == 0) {
    return iDate;
  } else if (offset < 0) {
    return prevDate(iDate, std::abs(offset));
  }
  time_t t = KT_Common::str2time(KT_Common::tostr(iDate * 1000000LL));
  t = t + ONE_DAY_SEC * offset;
  return KT_Common::strto<int>(KT_Common::tm2str(t, "%Y%m%d"));
}

int KT_Common::prevDate(int iDate, int offset) {
  if (offset == 0) {
    return iDate;
  } else if (offset < 0) {
...
int KT_Common::lastDate(int iDate, const char period) {
  if (period == 'D') {
    return iDate;
  }
  switch (period) {
...
inline bool KT_Common::matchPeriod(int lastDate, int date, const std::string &period) {
  ...
  if (period == "W") {
    int a = KT_Common::dateToWeek(KT_Common::tostr(lastDate));
    int b = KT_Common::dateToWeek(KT_Common::tostr(date));
    return a == b;
  } else if (period == "M") {
    int a = KT_Common::dateToMonth(KT_Common::tostr(lastDate));
    int b = KT_Common::dateToMonth(KT_Common::tostr(date));
    return a == b;
  } else if (period == "Q") {
    int a = KT_Common::dateToSeason(KT_Common::tostr(lastDate));
    int b = KT_Common::dateToSeason(KT_Common::tostr(date));
    return a == b;
  } else if (period == "S") {
    int a = KT_Common::dateToHalfYear(KT_Common::tostr(lastDate));
    int b = KT_Common::dateToHalfYear(KT_Common::tostr(date));
    return a == b;
  } else if (period == "Y") {
    int a = KT_Common::dateToYear(KT_Common::tostr(lastDate));
    int b = KT_Common::dateToYear(KT_Common::tostr(date));
    return a == b;
  }
  return false;
}

int KT_Common::getMatchPeriodDays(const std::vector<int> &days, const std::pair<string, int> &period,
                                  std::vector<int> &matchDays) {
  std::map<std::string, int> periodList{{"W", 5}, {"M", 25}, {"Q", 25 * 3}, {"S", 25 * 3 * 2}, {"Y", 256}};
  std::string p = KT_Common::upper(period.first);
