...
Current::~Current() {
  ...
  if (_reportStat) {
    ...
    } else if (_request.cPacketType == KANTONEWAY) {
      reportToStat("one_way_client");
    } else if (!_isKant && ServerConfig::ReportFlow) {
      //非kant客户端 从服务端上报调用信息
      reportToStat("not_kant_client");
    }
  }
}
...
const string &Current::getIp() const { return _data->ip(); }

int Current::getPort() const { return _data->port(); }

uint32_t Current::getUId() const { return _data->uid(); }

string Current::getServantName() const { return _request.sServantName; }

short Current::getRequestVersion() const { return _request.iVersion; }

map<string, string> &Current::getContext() { return _request.context; }

const map<string, string> &Current::getRequestStatus() const { return _request.status; }

string Current::getFuncName() const { return _request.sFuncName; }
...
int Current::getMessageType() const { return _request.iMessageType; }
...
void Current::setReportStat(bool bReport) { _reportStat = bReport; }
...
bool Current::isResponse() const { return _response; }

void Current::setCloseType(int type) { _data->setCloseType(type); }

int Current::getCloseType() const { return _data->closeType(); }

void Current::initialize(const shared_ptr<KT_EpollServer::RecvContext> &data) {
  _data = data;
  ...
  _isKant = _data->adapter()->isKantProtocol();
...
void Current::initializeClose(const shared_ptr<KT_EpollServer::RecvContext> &data) {
  _data = data;
...
void Current::sendResponse(const char *buff, uint32_t len) {
  shared_ptr<KT_EpollServer::SendContext> send = _data->createSendContext();
...
void Current::sendResponse(int iRet, const vector<char> &buff) {
  //单向调用不需要返回
  if (_request.cPacketType == KANTONEWAY) {
    return;
  }

  ResponsePacket response;
  response.sBuffer = buff;
  sendResponse(iRet, response, KANT_STATUS(), "");
}
...
void Current::sendResponse(int iRet, kant::KantOutputStream<kant::BufferWriterVector> &os) {
  ResponsePacket response;
  os.swap(response.sBuffer);
  sendResponse(iRet, response, KANT_STATUS(), "");
}

void Current::sendResponse(int iRet, tup::UniAttribute<kant::BufferWriterVector, kant::BufferReader> &attr) {
  ResponsePacket response;
  attr.encode(response.sBuffer);
  sendResponse(iRet, response, KANT_STATUS(), "");
}

void Current::sendResponse(int iRet, ResponsePacket &response, const map<string, string> &status,
  ...
  if (_request.iVersion != TUPVERSION) {
    ...
    response.cPacketType = KANTNORMAL;
    ...
    TLOGKANT("Current::sendResponse :" << response.iMessageType << "|" << _request.sServantName << "|"
                                       << _request.sFuncName << "|" << response.iRequestId << endl);

    response.writeTo(os);
  } else {
    //tup回应包用请求包的结构(这里和新版本KANT是有区别的)
    RequestPacket tupResponse;

    tupResponse.iRequestId = _request.iRequestId;
    tupResponse.iMessageType = _request.iMessageType;
    tupResponse.cPacketType = KANTNORMAL;
    ...
    if (tupResponse.sBuffer.size() == 0) {
      tup::UniAttribute<> kantAttr;
      kantAttr.setVersion(_request.iVersion);
      kantAttr.encode(response.sBuffer);
    }
    //iRet为0时,不记录在status里面,节省空间
    if (iRet != 0) {
      tupResponse.status[ServantProxy::STATUS_RESULT_CODE] = KT_Common::tostr(iRet);
    }
    ...
    TLOGKANT("Current::sendResponse :" << tupResponse.iMessageType << "|" << _request.sServantName << "|"
                                       << _request.sFuncName << "|" << tupResponse.iRequestId << endl);
...
ServantHandle *Current::getServantHandle() { return _servantHandle; }

KT_EpollServer::BindAdapter *Current::getBindAdapter() { return _data->adapter().get(); }
...
bool Current::isTraced() const { return _traceCall; }

string Current::getTraceKey() const { return _traceKey; }

bool Current::connectionExists() const { return _data->connectionExists(); }
