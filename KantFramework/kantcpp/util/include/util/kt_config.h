#ifndef _KT_CONFIG_H_
#define _KT_CONFIG_H_
...
* Field Separator
*/
const char KT_CONFIG_DOMAIN_SEP = '/';

/**
* 参数开始符
* Parameter Start Character
*/
const char KT_CONFIG_PARAM_BEGIN = '<';

/**
* 参数结束符
* Parameter Ender Character
*/
const char KT_CONFIG_PARAM_END = '>';

/**
* @brief 配置文件异常类.
* @brief Config File Exception Class
*/
struct KT_Config_Exception : public KT_Exception {
  KT_Config_Exception(const string &buffer) : KT_Exception(buffer){};
  KT_Config_Exception(const string &buffer, int err) : KT_Exception(buffer, err){};
  ~KT_Config_Exception() throw(){};
};
 ...
 */
struct KT_ConfigNoParam_Exception : public KT_Exception {
  KT_ConfigNoParam_Exception(const string &buffer) : KT_Exception(buffer){};
  ~KT_ConfigNoParam_Exception() throw(){};
};
...
*/
class KT_ConfigDomain {
 public:
  friend class KT_Config;
  ...
  /**
    ...
    */
  KT_ConfigDomain(const string &sLine);
  ...
  /**
     ...
     * @brief Copy
     *  
     * @param tcd 一个KT_ConfigDomain对象，指配置文件的一个域
     * @param tcd a KT_ConfigDomain object, refers to a domain of the configuration file
     */
  KT_ConfigDomain(const KT_ConfigDomain &tcd);
  ...
  /**
     ...
     * @param tcd a KT_ConfigDomain object, refers to a domain of the configuration file
     */
  KT_ConfigDomain &operator=(const KT_ConfigDomain &tcd);
  ...
  /**
    ...
    * @return KT_ConfigDomain*  指向子域的指针 
    * @return KT_ConfigDomain*  the pointer which points to the subdomain 
    * @throws KT_Config_Exception 
    */
  KT_ConfigDomain *addSubDomain(const string &name);
  ...
  /**
    ...
    * @param itBegin iterator, point to the button of the _domain
    * @return KT_ConfigDomain* 指向目标子域的指针
    * @return KT_ConfigDomain* a point point to the target subdomain
    */
  KT_ConfigDomain *getSubKtConfigDomain(vector<string>::const_iterator itBegin, vector<string>::const_iterator itEnd);
  const KT_ConfigDomain *getSubKtConfigDomain(vector<string>::const_iterator itBegin,
  ...
  /**
    ...
    * @return map
    */
  const map<string, KT_ConfigDomain *> &getDomainMap() const { return _subdomain; }
  ...
  /**
     ...
     * @brief Clone.
     * 
     * @return KT_ConfigDomain*
     */
  KT_ConfigDomain *clone() const { return new KT_ConfigDomain(*this); }
 ...
 protected:
  ...
  /**
    ...
    * Subdomain
    */
  map<string, KT_ConfigDomain *> _subdomain;
...
class KT_Config {
 public:
  ...
  /**
     ...
     * @brief Copy
     *  
     * @param kt为KT_Config类型
     */
  KT_Config(const KT_Config &kt);
  ...
  /**
     ...
     * @return KT_Config&
     */
  KT_Config &operator=(const KT_Config &kt);
...
Add domain. Add a domain under the current domain. If the sadddomain domain already exists, it is considered to be successful.
  ...
  /**
     ...
     */
  void joinConfig(const KT_Config &cf, bool bUpdate);
 ...
 protected:
  /**
    * @brief Parse输入流， 
    * 最终把输入流解析成为一个KT_ConfigDomain装入stack中 
    * @brief Parse Input Stream
    * Finally, the input stream is parsed into a KT_ConfigDomain and is loaded into the stack.
    * @param  要解析的输入流，按行解析 
  ...
  /**
    ...
    * @throws KT_Config_Exception 
    */
  KT_ConfigDomain *newKtConfigDomain(const string &sName);
  ...
  /**
    ...
    * @return value
    */
  KT_ConfigDomain *searchKtConfigDomain(const vector<string> &domains);
  const KT_ConfigDomain *searchKtConfigDomain(const vector<string> &domains) const;
