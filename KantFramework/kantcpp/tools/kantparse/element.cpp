...
/*************************************************************************************************/
const char *Builtin::builtinTable[] = {"void",  "bool",   "byte",   "short",  "int", "long",
                                       "float", "double", "string", "vector", "map"};

Builtin::Builtin(Kind kind, bool isUnsigned = false) : _kind(kind), _isUnsigned(isUnsigned) {}

Builtin::Kind Builtin::kind() const { return _kind; }
...
NamespacePtr Container::createNamespace(const string &id) {
  NamespacePtr np = std::dynamic_pointer_cast<Namespace>(shared_from_this());
  if (np) {
...
void Context::addInclude(const string &incl) {
  ...
  _includes.push_back(kant::KT_File::excludeFileExt(incl) + ".h");
}
