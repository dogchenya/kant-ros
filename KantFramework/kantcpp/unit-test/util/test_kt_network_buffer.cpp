...
TEST_F(UtilNetworkBufferTest, testIterator) {
  ...
  KT_NetWorkBuffer::buffer_iterator it = buff.begin();
...
TEST_F(UtilNetworkBufferTest, testFind) {
  ...
  it = std::search(buff.begin(), buff.end(), find.c_str(), find.c_str() + find.size(),
...
TEST_F(UtilNetworkBufferTest, testGetOrCreateBuffer) {
  ...
  shared_ptr<KT_NetWorkBuffer::Buffer> data;
...
TEST_F(UtilNetworkBufferTest, testCompact) {
  ...
  ASSERT_TRUE(KT_Port::strncasecmp(data->buffer(), &buff[10], data->length()) == 0);
}