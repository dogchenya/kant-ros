#ifndef _KT_SOCKET_H_
#define _KT_SOCKET_H_
...
* @brief socket异常类
*/
struct KT_Socket_Exception : public KT_Exception {
  KT_Socket_Exception(const string &buffer) : KT_Exception(buffer){};
  KT_Socket_Exception(const string &buffer, int err) : KT_Exception(buffer, err){};
  ~KT_Socket_Exception() throw(){};
};
...
class KT_Socket {
 public:
  ...
  /**
     ...
     * @param sPathName
     * @throws KT_Socket_Exception:其他错误还是通过异常返回(例如),例如地址错误
     * @return int
     */
  ...
  /**
    ...
    */
  SOCKET_TYPE accept(KT_Socket &tcSock, struct sockaddr *pstSockAddr, SOCKET_LEN_TYPE &iSockLen);
...
#undef IPv6_ADDRESS_CHAR
  ...
  /**
     ...
     * @return
     */
  static KT_Socket::addr_type createSockAddr(const char *addr);
  ...
  /**
	 ...
	 * @return
	 */
  static KT_Socket::addr_type createSockAddr(const KT_Endpoint &ep);
  ...
  static void closeSocketNoThrow(int);
    ...
    *  
    */
    KT_Socket(const KT_Socket &tcSock);

    /**
    * @brief  no implementation.
    */
    KT_Socket& operator=(const KT_Socket &tcSock);
#endif
