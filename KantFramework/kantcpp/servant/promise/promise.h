#ifndef _KANT_PROMISE_H_
#define _KANT_PROMISE_H_
...
template <typename T>
struct IsFutureType : std::false_type {};
// only Future<T> is a future type.
template <typename T>
struct IsFutureType<Future<T>> : std::true_type {};
...
struct FutureTypeTraits {
  ...
  static void init(StorageType& s, RValueType v) { s.reset(new T(v)); }

  static void assign(DestType d, const StorageType& s) { d = *s; }
};
...
struct FutureTypeTraits<T&> {
  ...
  static void init(StorageType& s, RValueType v) { s = &v; }
...
template <typename T>
class PromptFutureImpl final : public FutureInterface<T>, public std::enable_shared_from_this<PromptFutureImpl<T>> {
 public:
  ...
  PromptFutureImpl(RValueType v) : m_value(), m_exp() { FutureTypeTraits<T>::init(m_value, v); }

  PromptFutureImpl(ExceptionPtr exp) : m_value(), m_exp(exp) {}
  ...
  bool hasException() const override { return (!!m_exp); }

  LValueType get() const override {
    if (m_exp) m_exp->rethrow();
    return *m_value;
  }
  ...
  void appendWacther(const Watcher& watcher) override {
    try {
      if (watcher) watcher.run(this->shared_from_this());
    } catch (...) {
    }
  }
...
template <>
class PromptFutureImpl<void> final : public FutureInterface<void>,
 ...
 public:
  ...
  PromptFutureImpl() : m_exp() {}

  PromptFutureImpl(ExceptionPtr exp) : m_exp(exp) {}
...
class FutureInternal {
 public:
  ...
  void markFinishWithException(const ExceptionPtr& exp) {
    {
      std::lock_guard<std::mutex> lock(m_mutex);
      if (m_isDone) throw Exception("Duplicated mark finish with exception.");
 ...
 protected:
  FutureInternal() : m_mutex(), m_cv(), m_isDone(false), m_exp() {}
...
class FutureImpl : public FutureInterface<T>,
 ...
 public:
  ...
  bool tryGet(DestType d) const override {
    if (!FutureInternal::isDone()) return false;
  ...
  void setValue(RValueType v) override {
    {
      ...
      std::lock_guard<std::mutex> lock(m_mutex);
      if (m_isDone) throw Exception("Duplicated set value.");
...
class FutureImpl<void>
  : public FutureInterface<void>, public std::enable_shared_from_this<FutureImpl<void>>, private FutureInternal {
 public:
  using RValueType = typename FutureInterface<void>::RValueType;
  using LValueType = typename FutureInterface<void>::LValueType;
  using Watcher = typename FutureInterface<void>::Watcher;

  FutureImpl() : m_watchers() {}
  ...
  LValueType get() const override { wait(); }

  void set() override {
    {
      // FixMe: move this to FutureInternal.
      std::lock_guard<std::mutex> lock(m_mutex);
      if (m_isDone) throw Exception("Duplicated set finish.");
...
class SequentialWatcher {
 ...
 public:
  SequentialWatcher(const Watcher& w, const Promise<ValueType>& p) : m_watcher(w), m_promise(p) {}
...
class ForwardWatcher {
 public:
  ForwardWatcher(const Promise<R>& p) : m_promise(p) {}
...
class FutureBase {
 public:
  ...
  FutureBase(ExceptionPtr exp) : m_future(new PromptFutureImpl<T>(exp)) {}

  virtual ~FutureBase() {}

  LValueType get() const {
    if (!m_future) throw Exception("future uninitialized");
    return m_future->get();
  }

  bool isDone() const {
    if (!m_future) return false;
  ...
  // c++11 standard
  explicit operator bool() const { return m_future ? true : false; }
...
class Future : public promise::FutureBase<T> {
 public:
  ...
  Future(ExceptionPtr exp) : promise::FutureBase<T>(exp) {}
  ...
  Future<typename promise::UnderlyType<R>::Type> then(const Callback<R(const Future<T>&)>& watcher) {
    ...
    if (!this->m_future) throw Exception("future uninitialized");
 ...
 private:
  ...
  Future(const FuturePtr& future) : promise::FutureBase<T>(future) {}

  template <typename U, typename V>
  friend class promise::SequentialWatcher;
  friend class Promise<T>;
};

template <>
class Future<void> : public promise::FutureBase<void> {
 public:
  // can be initialize in tuple.
  Future() = default;

  Future(ExceptionPtr exp) : promise::FutureBase<void>(exp) {}
 ...
 private:
  ...
  Future(const FuturePtr& future) : promise::FutureBase<void>(future) {}
...
class Promise final {
 public:
  ...
  Promise() : m_future(new promise::FutureImpl<ValueType>()) {}

  void setValue(typename promise::FutureTypeTraits<T>::RValueType v) { m_future->setValue(v); }

  void setException(const ExceptionPtr& e) { m_future->setException(e); }

  Future<ValueType> getFuture() { return m_future; }
...
class Promise<void> final {
 public:
  Promise() : m_future(new promise::FutureImpl<void>()) {}

  void set() { m_future->set(); }

  void setException(const ExceptionPtr& e) { m_future->setException(e); }

  Future<void> getFuture() { return m_future; }
