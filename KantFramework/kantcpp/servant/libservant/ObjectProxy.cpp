...
///////////////////////////////////////////////////////////////////////////////////
ObjectProxy::ObjectProxy(CommunicatorEpoll *pCommunicatorEpoll, ServantProxy *servantProxy,
...
void ObjectProxy::initialize(bool rootServant) {
  _endpointManger.reset(
    new EndpointManager(this, _communicatorEpoll->getCommunicator(), _communicatorEpoll->isFirstNetThread()));
...
const vector<AdapterProxy *> &ObjectProxy::getAdapters() { return _endpointManger->getAdapters(); }
...
void ObjectProxy::invoke(ReqMessage *msg) {
  TLOGKANT("[ObjectProxy::invoke, objname:" << _name << ", begin...]" << endl);
  ...
  if (bFirst) {
    ...
    //把数据缓存在obj里面
    TLOGKANT("[ObjectProxy::invoke, objname:" << _name << ", select adapter proxy not valid (have not invoke reg)]"
                                              << endl);
  ...
  if (!pAdapterProxy) {
    ...
    msg->response->iRet = KANTADAPTERNULL;
  ...
  if (!msg->adapter->trans()->hasConnected()) {
    bool bRet = _reqTimeoutQueue.push(msg, this->_servantProxy->kant_connect_timeout() + msg->iBeginTime);

    assert(bRet);

    //把数据缓存在obj里面
    TLOGKANT("[ObjectProxy::invoke, " << _name << ", select adapter proxy not connected (have not invoke reg)]"
                                      << endl);
    return;
  }
...
void ObjectProxy::prepareConnection(AdapterProxy *adapterProxy) {
  while (!_reqTimeoutQueue.empty()) {
    TLOGKANT("[ObjectProxy::doInvoke, " << _name << ", conection queue pop size:" << _reqTimeoutQueue.size() << "]"
                                        << endl);
...
void ObjectProxy::onConnect(AdapterProxy *adapterProxy) { prepareConnection(adapterProxy); }
...
void ObjectProxy::doInvoke() {
  ...
  TLOGKANT("[ObjectProxy::doInvoke, objname:" << _name << ", begin...]" << endl);
...
void ObjectProxy::doTimeout() {
  ...
  while (_reqTimeoutQueue.timeout(reqInfo)) {
    reqInfo->response->iRet = KANTINVOKETIMEOUT;
