...

{
  ...
  //节点队列未发送请求的大小限制
  _noSendQueueLimit = KT_Common::strto<size_t>(
    pCommunicator->getProperty("sendqueuelimit", pCommunicator->getProperty("nosendqueuelimit", "100000")));
  if (_noSendQueueLimit < 1000) {
    _noSendQueueLimit = 1000;
  }

  //检查超时请求的时间间隔，单位:ms
  _timeoutCheckInterval = KT_Common::strto<int64_t>(pCommunicator->getProperty("timeoutcheckinterval", "1000"));
...
void CommunicatorEpoll::handleTerminate() {
  ...
  if (_scheduler) {
    ...
    if (_epoller) {
      //定时任务都删除掉
      for_each(_timerIds.begin(), _timerIds.end(), [&](int64_t id) { _epoller->erase(id); });
    }
...
void CommunicatorEpoll::notifyUpdateEndpoints(ServantProxy *servantProxy, const set<EndpointInfo> &active,
                                              const set<EndpointInfo> &inactive) {
  CommunicatorEpoll *ce = this;

  _epoller->asyncCallback([=]() { servantProxy->onNotifyEndpoints(ce, active, inactive); });
}
...
ObjectProxy *CommunicatorEpoll::servantToObjectProxy(ServantProxy *servantProxy) {
  KT_ThreadRLock lock(_servantMutex);
...
ObjectProxy *CommunicatorEpoll::hasObjectProxy(const string &sObjectProxyName, const string &setName) {
  KT_LockT<KT_ThreadRecMutex> lock(_objectMutex);
...
ObjectProxy *CommunicatorEpoll::createObjectProxy(ServantProxy *servantProxy, const string &sObjectProxyName,
  ...
  {
    KT_ThreadWLock lock(_vObjectMutex);
  ...
  {
    KT_ThreadWLock lock(_servantMutex);
...
void CommunicatorEpoll::addFd(AdapterProxy *adapterProxy) {
  shared_ptr<KT_Epoller::EpollInfo> epollInfo = adapterProxy->trans()->getEpollInfo();
...
bool CommunicatorEpoll::handleCloseImp(const shared_ptr<KT_Epoller::EpollInfo> &data) {
  assert(_threadId == this_thread::get_id());
...
bool CommunicatorEpoll::handleInputImp(const shared_ptr<KT_Epoller::EpollInfo> &data) {
  assert(_threadId == this_thread::get_id());
  ...
  try {
    ...
    TLOGKANT("[CommunicatorEpoll::handleInputImp] error:" << e.what() << endl);
...
bool CommunicatorEpoll::handleOutputImp(const shared_ptr<KT_Epoller::EpollInfo> &data) {
  assert(_threadId == this_thread::get_id());
  ...
  try {
    ...
    TLOGKANT("[CommunicatorEpoll::handleOutputImp] error:" << e.what() << endl);
    adapterProxy->addConnExc(true);
    return false;
  }
...
bool CommunicatorEpoll::popStatMsg(StatReport::MapStatMicMsg *&mStatMsg) { return _statQueue.pop_front(mStatMsg); }
...
void CommunicatorEpoll::getResourcesInfo(ostringstream &desc) {
  ...
  desc << KT_Common::outfill("index") << _netThreadSeq << endl;
  desc << KT_Common::outfill("stat size") << getReportSize() << endl;
  desc << KT_Common::outfill("obj num") << getObjNum() << endl;

  const static string TAB = "    ";
  for (size_t i = 0; i < getObjNum(); ++i) {
    desc << TAB << OUT_LINE_TAB(1) << endl;

    desc << TAB << KT_Common::outfill("obj name") << getObjectProxy(i)->name() << endl;
    const vector<AdapterProxy *> &adapters = getObjectProxy(i)->getAdapters();

    for (auto adapter : adapters) {
      desc << TAB << TAB << OUT_LINE_TAB(2) << endl;

      desc << TAB << TAB << KT_Common::outfill("adapter") << adapter->endpoint().getEndpoint().toString() << endl;
      desc << TAB << TAB << KT_Common::outfill("recv size") << adapter->trans()->getRecvBuffer().getBufferLength()
           << endl;
      desc << TAB << TAB << KT_Common::outfill("send size") << adapter->trans()->getSendBuffer().getBufferLength()
           << endl;
    }
  }
}
...
bool CommunicatorEpoll::handleNotify(const shared_ptr<KT_Epoller::EpollInfo> &data) {
  assert(_threadId == this_thread::get_id());
...
void CommunicatorEpoll::initializeEpoller() {
  ...
  _scheduler = KT_CoroutineScheduler::scheduler();
...
void CommunicatorEpoll::run() {
  ...
  _epoller->setName("communicator-epoller-public-netseq:" + KT_Common::tostr(_netThreadSeq));
