#ifndef _KT_CAS_QUEUE_H_
#define _KT_CAS_QUEUE_H_
...
class KT_CasQueue {
 ...
 protected:
  KT_CasQueue(const KT_CasQueue&) = delete;
  KT_CasQueue(KT_CasQueue&&) = delete;
  KT_CasQueue& operator=(const KT_CasQueue&) = delete;
  KT_CasQueue& operator=(KT_CasQueue&&) = delete;
...
template <typename T, typename D>
bool KT_CasQueue<T, D>::pop_front(T& t) {
  KT_LockT<KT_SpinLock> lock(_mutex);
  if (_queue.empty()) {
    return false;
  }
...
template <typename T, typename D>
bool KT_CasQueue<T, D>::pop_front() {
  KT_LockT<KT_SpinLock> lock(_mutex);
  if (_queue.empty()) {
    return false;
  }
...
template <typename T, typename D>
void KT_CasQueue<T, D>::push_back(const T& t) {
  KT_LockT<KT_SpinLock> lock(_mutex);
  _queue.push_back(t);
  ++_size;
}
...
template <typename T, typename D>
void KT_CasQueue<T, D>::push_back(const queue_type& qt) {
  KT_LockT<KT_SpinLock> lock(_mutex);
...
template <typename T, typename D>
void KT_CasQueue<T, D>::push_front(const T& t) {
  KT_LockT<KT_SpinLock> lock(_mutex);
...
template <typename T, typename D>
void KT_CasQueue<T, D>::push_front(const queue_type& qt) {
  KT_LockT<KT_SpinLock> lock(_mutex);
...
template <typename T, typename D>
bool KT_CasQueue<T, D>::swap(queue_type& q) {
  KT_LockT<KT_SpinLock> lock(_mutex);
...
template <typename T, typename D>
size_t KT_CasQueue<T, D>::size() const {
  KT_LockT<KT_SpinLock> lock(_mutex);
  return _size;
}

template <typename T, typename D>
void KT_CasQueue<T, D>::clear() {
  KT_LockT<KT_SpinLock> lock(_mutex);
  _queue.clear();
  _size = 0;
}
...
template <typename T, typename D>
bool KT_CasQueue<T, D>::empty() const {
  KT_LockT<KT_SpinLock> lock(_mutex);
  return _queue.empty();
}
