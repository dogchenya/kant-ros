...
class TaskList : public KT_ThreadMutex {
 public:
  /**
     * 构造函数
     */
...
class TaskListElegant : public TaskList {
 ...
 protected:
  KT_ThreadPool _poolMaster;
};

class ExecuteTask : public KT_Singleton<ExecuteTask>, public KT_ThreadLock, public KT_Thread {
 public:
  /**
     * 添加任务请求
     * 
  ...
  unsigned int getElegantWaitSecond() const { return _elegantWaitSecond; }

  void terminate();

 public:
  ExecuteTask();
  void init(KT_Config *conf);
  AdminRegistryImp *getAdminImp() { return _adminImp; }
  ~ExecuteTask();
  virtual void run();

 protected:
  KT_ThreadMutex _initLock;
