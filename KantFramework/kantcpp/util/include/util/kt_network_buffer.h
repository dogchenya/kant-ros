 ...
 *
 * KT_NetWorkBuffer说明:
 * - KT_NetWorkBuffer主要用于网络的收发, 设计的目标是减少内存copy
 * - Buffer对象是KT_NetWorkBuffer子类, 描述了一个完整连续的空间的buffer
 * - KT_NetWorkBuffer是由多个Buffer连接到一起构成
 *
 * Buffer说明:
 * - Buffer描述了一整块连续的内存
...
* @brief
*/
struct KT_NetWorkBuffer_Exception : public KT_Exception {
  KT_NetWorkBuffer_Exception(const string &sBuffer) : KT_Exception(sBuffer){};
  ~KT_NetWorkBuffer_Exception(){};
};

class KT_NetWorkBuffer {
 public:
  ////////////////////////////////////////////////////////////////////////////
  /**
	 * 定义协议解析的返回值
  ...
  enum PACKET_TYPE {
    ...
    PACKET_FULL = 1,
    PACKET_FULL_CLOSE =
      2,  ///< 收到完整包，并需要结束session，如: http close情况, 只在客户端协议解析中生成, 服务端相当于PACKET_FULL
    PACKET_ERR = -1,
  };
  ...
  /**
	 ...
	 */
  typedef std::function<PACKET_TYPE(KT_NetWorkBuffer &, vector<char> &)> protocol_functor;
  ...
  class Buffer {
   public:
    ...
    /**
		 ...
		 * @param buff
		 */
    inline void setBuffer(const vector<char> &buff) { setBuffer(buff.data(), buff.size()); }

    /**
		 * 设置buffer, 数据copy
		 * @param buff
		 */
    inline void setBuffer(const string &buff) { setBuffer(buff.c_str(), buff.size()); }
    ...
    inline char &at(size_t offset) {
      if (_readIdx + offset >= _writeIdx)
        throw KT_NetWorkBuffer_Exception("[KT_NetWorkBuffer::Buffer] at '" + KT_Common::tostr(offset) +
                                         "' offset overflow");
    ...
    inline void addReadIdx(uint32_t len) {
      if (_readIdx + len > _writeIdx) {
        throw KT_NetWorkBuffer_Exception("[KT_NetWorkBuffer::Buffer::addReadIdx] len:" + KT_Common::tostr(len) +
                                         " overflow.");
      }
    ...
    inline void addWriteIdx(uint32_t len) {
      if (len > left()) {
        throw KT_NetWorkBuffer_Exception("[KT_NetWorkBuffer::Buffer::addWriteIdx] len:" + KT_Common::tostr(len) +
                                         " overflow.");
      }
  ...
  class buffer_iterator : public std::iterator<std::random_access_iterator_tag, char> {
   public:
    buffer_iterator(const KT_NetWorkBuffer *buffer, size_t offset) : _buffer(buffer) { parseOffset(offset); }
    ...
    char &operator*() { return (*_it)->buffer()[_pos]; }

    char *operator->() { return &(*_it)->buffer()[_pos]; }

    char &operator*() const { return (*_it)->buffer()[_pos]; }

    const char *operator->() const { return &(*_it)->buffer()[_pos]; }

    buffer_iterator operator+(size_t n) const { return buffer_iterator(_buffer, _offset + n); }
    ...
    difference_type operator-(const buffer_iterator &n) const { return _offset - n._offset; }

    buffer_iterator operator-(size_t n) const { return buffer_iterator(_buffer, _offset - n); }

    bool operator<(const buffer_iterator n) const { return _offset < n._offset; }
    ...
    buffer_iterator &operator++() {
      if (_offset >= _buffer->getBufferLength()) return *this;
   ...
   protected:
    const KT_NetWorkBuffer *_buffer;
  ...
  /**
	 ...
	 * @param buff
	 */
  KT_NetWorkBuffer(void *connection) { _connection = connection; }
  ...
  void setContextData(void *contextData,
                      std::function<void(KT_NetWorkBuffer *)> deconstruct = std::function<void(KT_NetWorkBuffer *)>()) {
  ...
  T iteratorToIterator(const buffer_iterator &sit, const buffer_iterator &eit) {
    ...
    if (sit > eit) {
      throw KT_NetWorkBuffer_Exception("[KT_NetWorkBuffer::iteratorToIterator] sit > eit error");
    }
  ...
  /**
	 ...
	 * @param len
	 * @return 不够会抛异常KT_NetWorkBuffer_Exception
	 */
  template <typename T>
  T getHeader(size_t len) const {
    if (getBufferLength() < len) {
      throw KT_NetWorkBuffer_Exception("[KT_NetWorkBuffer::getHeader] no enough buff(" +
                                       KT_Common::tostr(getBufferLength()) + ") to get(" + KT_Common::tostr(len) + ")");
    }
  ...
  /**
	 ...
	 * @return
	 */
  KT_NetWorkBuffer::PACKET_TYPE checkHttp();
  ...
  static KT_NetWorkBuffer::PACKET_TYPE parseBinary1(KT_NetWorkBuffer &in, vector<char> &out) {
  ...
  static KT_NetWorkBuffer::PACKET_TYPE parseBinary2(KT_NetWorkBuffer &in, vector<char> &out) {
  ...
  static KT_NetWorkBuffer::PACKET_TYPE parseBinary4(KT_NetWorkBuffer &in, vector<char> &out) {
  ...
  /**
	 ...
	 * @return
	 */
  static KT_NetWorkBuffer::PACKET_TYPE parseHttp(KT_NetWorkBuffer &in, vector<char> &out);
  ...
  /**
	 ...
	 * @return
	 */
  static KT_NetWorkBuffer::PACKET_TYPE parseEcho(KT_NetWorkBuffer &in, vector<char> &out);
 ...
 protected:
  ...
  template <typename T>
  KT_NetWorkBuffer::PACKET_TYPE parseBuffer(vector<char> &buffer, T minLength, T maxLength) {
    if (getBufferLength() < sizeof(T)) {
      return PACKET_LESS;
    }

    if (minLength < sizeof(T)) minLength = sizeof(T);
 ...
 protected:
  ...
  /**
	 ...
	 */
  std::function<void(KT_NetWorkBuffer *)> _deconstruct;
