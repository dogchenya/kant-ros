#ifndef __KANT_ENDPOINT_MANAGER_H_
#define __KANT_ENDPOINT_MANAGER_H_
...
#include "util/kt_consistent_hash_new.h"
 ...
 * 获取路由的方式
 */
enum GetEndpointType { E_DEFAULT = 0, E_ALL = 1, E_SET = 2, E_STATION = 3 };
...
class QueryEpBase : public QueryFPrxCallback {
 public:
  ...
  /*
     * 获取所有节点信息的回调处理
     */
  void callback_findObjectById4All(kant::Int32 ret, const vector<kant::EndpointF> &activeEp,
                                   const vector<kant::EndpointF> &inactiveEp);

  /*
     * 获取所有节点信息的异常回调处理
     */
  void callback_findObjectById4All_exception(kant::Int32 ret);

  /*
     * 获取所有节点信息的回调处理
     */
  void callback_findObjectById4Any(kant::Int32 ret, const vector<kant::EndpointF> &activeEp,
                                   const vector<kant::EndpointF> &inactiveEp);

  /*
     * 获取所有节点信息的异常回调处理
     */
  void callback_findObjectById4Any_exception(kant::Int32 ret);

  /*
     * 按idc获取的节点信息的回调处理
     */
  void callback_findObjectByIdInSameGroup(kant::Int32 ret, const vector<kant::EndpointF> &activeEp,
                                          const vector<kant::EndpointF> &inactiveEp);

  /*
     * 按idc分组获取的节点信息的异常回调处理
     */
  void callback_findObjectByIdInSameGroup_exception(kant::Int32 ret);

  /*
     * 按set获取的节点信息的回调处理
     */
  void callback_findObjectByIdInSameSet(kant::Int32 ret, const vector<kant::EndpointF> &activeEp,
                                        const vector<kant::EndpointF> &inactiveEp);

  /*
     * 按set获取的节点信息的异常回调处理
     */
  void callback_findObjectByIdInSameSet_exception(kant::Int32 ret);

  /*
     * 按地区获取的节点信息的回调处理
     */
  void callback_findObjectByIdInSameStation(kant::Int32 ret, const vector<kant::EndpointF> &activeEp,
                                            const vector<kant::EndpointF> &inactiveEp);

  /*
     * 按地区获取的节点信息的异常回调处理
     */
  void callback_findObjectByIdInSameStation_exception(kant::Int32 ret);
 ...
 private:
  ...
  /*
     * 主控的请求的响应到了,做相应的处理
     */
  void doEndpoints(const vector<EndpointF> &activeEp, const vector<EndpointF> &inactiveEp, int iRet,
                   bool bSync = false);
...
class EndpointManager : public QueryEpBase {
 ...
 public:
  ...
  /**
     * 获取所有的服务节点
     */
  const vector<AdapterProxy *> &getAdapters() { return _vAllProxys; }
 ...
 private:
  ...
  /*
     ...
     */
  void updateConHashProxyWeighted(bool bStatic, map<string, AdapterProxy *> &mLastConHashProxys,
 ...
 private:
  ...
  /*
     * 一致性hash静态权重时使用
     */
  KT_ConsistentHashNew _consistentHashWeight;
  ...
  /*
     * 一致性hash普通使用
     */
  KT_ConsistentHashNew _consistentHash;
...
class EndpointThread : public QueryEpBase {
 public:
  /*
     * 构造函数
     */
  EndpointThread(Communicator *pComm, const string &sObjName, GetEndpointType type, const string &sSetName,
                 bool bFirstNetThread = false);
  ...
  /*
     * 用KT_Endpoint存在可用与不可用的节点信息
     */
  void getTCEndpoints(vector<KT_Endpoint> &activeEndPoint, vector<KT_Endpoint> &inactiveEndPoint);
 ...
 private:
  ...
  vector<KT_Endpoint> _activeTCEndPoint;

  /*
     * 不活跃的结点
     */
  vector<EndpointInfo> _inactiveEndPoint;
  vector<KT_Endpoint> _inactiveTCEndPoint;
};
...
class EndpointManagerThread {
 public:
  ...
  /*
     * 根据set获取可用与不可用的结点
     */
  void getEndpointBySet(const string &sName, vector<EndpointInfo> &activeEndPoint,
                        vector<EndpointInfo> &inactiveEndPoint);

  /*
     * 根据地区获取可用与不可用的结点
     */
  void getEndpointByStation(const string &sName, vector<EndpointInfo> &activeEndPoint,
                            vector<EndpointInfo> &inactiveEndPoint);

  /*
     * 按idc获取可用与不可用的结点
     */
  void getTCEndpoint(vector<KT_Endpoint> &activeEndPoint, vector<KT_Endpoint> &inactiveEndPoint);

  /*
     * 获取所有可用与不可用的结点
     */
  void getTCEndpointByAll(vector<KT_Endpoint> &activeEndPoint, vector<KT_Endpoint> &inactiveEndPoint);

  /*
     * 根据set获取可用与不可用的结点
     */
  void getTCEndpointBySet(const string &sName, vector<KT_Endpoint> &activeEndPoint,
                          vector<KT_Endpoint> &inactiveEndPoint);

  /*
     * 根据地区获取可用与不可用的结点
     */
  void getTCEndpointByStation(const string &sName, vector<KT_Endpoint> &activeEndPoint,
                              vector<KT_Endpoint> &inactiveEndPoint);
