...
KT_SpinLock KT_GrpcServer::_mutex;

unordered_map<int32_t, shared_ptr<KT_GrpcServer>> KT_GrpcServer::_http2;
...
static ssize_t send_callback(nghttp2_session *session, const uint8_t *data, size_t length, int flags, void *user_data) {
  KT_GrpcServer *ptr = (KT_GrpcServer *)user_data;
...
int KT_GrpcServer::encodeResponse(const shared_ptr<KT_GrpcServer::Http2Context> &context, std::string gStatus,
                                  vector<char> &out) {
  ...
  data_prd.read_callback = server::str_read_callback;
  {
    ...
    _err = nghttp2_submit_headers(_session, NGHTTP2_FLAG_NONE, context->reqId, nullptr, hdrs,
                                  context->response.getHeaders().size() + 1, nullptr);
    if (_err != 0) {
      delete[] hdrs;
      return _err;
    }
    ...
    _err = nghttp2_submit_headers(_session, NGHTTP2_FLAG_END_STREAM, context->reqId, nullptr, hdr, 1, nullptr);
    if (_err != 0) {
      delete[] hdrs;
      return _err;
    }
...
void KT_GrpcServer::packGrpcResponse(shared_ptr<KT_GrpcServer::Http2Context> &context, const int status,
                                     const string &body) {
  context->response.setHeader("content-type", "application/grpc");
  context->response.setResponse(status, "OK", body);
}

shared_ptr<KT_GrpcServer> KT_GrpcServer::getHttp2(uint32_t uid) {
...
void KT_GrpcServer::addHttp2(uint32_t uid, const shared_ptr<KT_GrpcServer> &ptr) {
  KT_LockT<KT_SpinLock> lock(_mutex);

  _http2[uid] = ptr;
}

void KT_GrpcServer::delHttp2(uint32_t uid) {
  KT_LockT<KT_SpinLock> lock(_mutex);
...
KT_NetWorkBuffer::PACKET_TYPE KT_GrpcServer::parseGrpc(KT_NetWorkBuffer &in, vector<char> &out) {
  KT_GrpcServer *sessionPtr = (KT_GrpcServer *)(in.getContextData());
  KT_EpollServer::Connection *connection = (KT_EpollServer::Connection *)in.getConnection();

  if (sessionPtr == NULL) {
    shared_ptr<KT_GrpcServer> session(new KT_GrpcServer());
...
namespace client {
static ssize_t send_callback(nghttp2_session *session, const uint8_t *data, size_t length, int flags, void *user_data) {
  KT_GrpcClient *nghttp2 = (KT_GrpcClient *)user_data;
...
int KT_GrpcClient::submit(const KT_HttpRequest &request) {
...
KT_NetWorkBuffer::PACKET_TYPE KT_GrpcClient::parseResponse(KT_NetWorkBuffer &in,
