#ifndef _KT_CPP_TRANSCEIVER_H_
#define _KT_CPP_TRANSCEIVER_H_
...
* @brief 连接异常类
*/
struct KT_Transceiver_Exception : public KT_Exception {
  KT_Transceiver_Exception(const string &sBuffer) : KT_Exception(sBuffer){};
  KT_Transceiver_Exception(const string &sBuffer, int err) : KT_Exception(sBuffer, err){};
  ~KT_Transceiver_Exception() throw(){};
};

/**
 * 网络传输基类
 * 使用方式说明:
 * 1 通过暴露注册回调的机制来和使用方交互, 配和epoller类完成网路层的收发都统一, epoller对象需要使用者自己创建和管理, 并传递给KT_Transceiver
 * 2 即可支持客户端的网络, 也可以支持服务端的网络, 但是两者初始化过程是有区别的(initializeClient & initializeServer)
 *   客户端网络: 发送connect时, 会创建句柄, 有oncreate_callback调用到业务层, 通过参数将KT_Epoller::EpollInfo*给业务层, 业务层在回调中根据实际情况完成epoll事件函数的注册
 *   服务端网络: 调用者完成accept, 然后通过bindFd的形式, 将句柄传递进来, bindFd会创建KT_Epoller::EpollInfo*, 返回给调用方
 * 3 无论是客户端网络还是服务端网络, 使用者都不用管理KT_Epoller::EpollInfo*的生命周期, 它会被KT_Transceiver管理, 连接关闭时释放
 * 4 作为客户端, 支持sock4/sock5/http代理(通过oncreate_callback回调, 业务层将代理信息KT_ProxyInfo返回给KT_Transceiver), 也可以通过setBindAddr指定客户端发包时的绑定地址
 * 5 在数据包的管理上, KT_Transceiver内只保存最后一个发送的数据包(可能发送了一半), 因此调用者需要维持发送的数据包队列, 几个回调函数的说明:
 *   oncreate_callback: 只存在于客户端模式, 当KT_Transceiver创建完客户端fd时回调oncreate_callback, 使用者在回调事件中监听fd事件, 如果有代理, 则返回代理信息, 否则返回null(注意, 如果有, 则没法都要new一个新的, 因为代理是具备状态信息的)
 *   onconnect_callback: 建立连接以后的回调, 注意对于ssl, 是指已经完成ssl握手后的回调
 *   onsend_callback: 当KT_Transceiver内的数据都发送完毕, 则回调onsend_callback, 使得调用者可以发送下一个数据
...
class KT_Transceiver {
 public:
  ...
  using oncreate_callback = std::function<shared_ptr<KT_ProxyInfo>(KT_Transceiver *)>;
  //开启openssl对象的回调
  using onopenssl_callback = std::function<std::shared_ptr<KT_OpenSSL>(KT_Transceiver *)>;
  //关闭句柄的回调
  using onclose_callback = std::function<void(KT_Transceiver *, CloseReason reason, const string &err)>;
  //建立连接上的回调(只对客户端有效)
  using onconnect_callback = std::function<void(KT_Transceiver *)>;
  //可以发出业务请求了(对客户端: 连接建立完成, 对SSL: 握手完成, 可以发业务数据的回调)
  using onrequest_callback = std::function<void(KT_Transceiver *)>;
  //解析协议的回调
  using onparser_callback = std::function<KT_NetWorkBuffer::PACKET_TYPE(KT_NetWorkBuffer &, KT_Transceiver *)>;
  //完整解析完一个包之后的回调
  using oncompletepackage_callback = std::function<void(KT_Transceiver *)>;
  //完整一次网络解析之后回调(一般有多次解析onparser_callback 以及 oncompletepackage_callback 之后回调, 通常在业务层可以在这个函数中可以把解析的数据一次性写入到队列中)
  using oncompletenetwork_callback = std::function<void(KT_Transceiver *)>;
  //cient侧: 发送鉴权包的回调, 业务层在回调里面组织鉴权包
  using onclientsendauth_callback = std::function<shared_ptr<KT_NetWorkBuffer::Buffer>(KT_Transceiver *)>;
  //client侧: 收到鉴权包的的回调, 业务层解包(注意返回PACKET_FULL, 表示鉴权成功)
  using onclientverifyauth_callback =
    std::function<KT_NetWorkBuffer::PACKET_TYPE(KT_NetWorkBuffer &, KT_Transceiver *)>;
  //server侧: 验证鉴权包并返回验证包的回调
  using onserververifyauth_callback =
    std::function<pair<KT_NetWorkBuffer::PACKET_TYPE, shared_ptr<KT_NetWorkBuffer::Buffer>>(KT_NetWorkBuffer &,
                                                                                            KT_Transceiver *)>;
  ...
  /**
     ...
     * @param ep, 服务端地址 
     */
  KT_Transceiver(KT_Epoller *epoller, const KT_Endpoint &ep);

  /**
     *
     *析构函数
     */
  virtual ~KT_Transceiver();
  ...
  /**
     ...
     * 句柄是connect时创建的
     */
  void initializeClient(const oncreate_callback &oncreate, const onclose_callback &onclose,
  ...
  /**
     ...
     * 句柄是外部accept后, 传递进去的
     */
  void initializeServer(const onclose_callback &onclose, const onrequest_callback &onrequest,
                        const onparser_callback &onparser, const onopenssl_callback &onopenssl,
                        const oncompletepackage_callback &onfinish = oncompletepackage_callback(),
                        const oncompletenetwork_callback &onfinishAll = oncompletenetwork_callback());
  ...
  /**
     ...
     * @param bindAddr
     */
  void setBindAddr(const KT_Socket::addr_type &bindAddr);

  /**
     * 获取client地址, 如果是udp, 则表示最后一个包对应的client地址
     * @return
     */
  const KT_Socket::addr_type &getClientAddr() const { return _clientAddr; }

  /**
     * 获取server地址(如果是客户端, 并设置了代理, 则返回代理地址)
     * @return
     */
  const KT_Socket::addr_type &getServerAddr() const { return _serverAddr; }
  ...
  /**
     ...
     * 并且会维持句柄的持有
     * @param fd
     * @return KT_Epoller::EpollInfo*
     */
  shared_ptr<KT_Epoller::EpollInfo> bindFd(int fd);
  ...
  /**
	 ...
	 * @return
	 */
  inline KT_NetWorkBuffer &getSendBuffer() { return _sendBuffer; }

  /**
	 * recv buffer
	 * @return
	 */
  inline KT_NetWorkBuffer &getRecvBuffer() { return _recvBuffer; }

  /**
     * get epoll info
     */
  inline const shared_ptr<KT_Epoller::EpollInfo> &getEpollInfo() { return _epollInfo; }
  ...
  /**
     ...
     */
  virtual ReturnStatus sendRequest(const shared_ptr<KT_NetWorkBuffer::Buffer> &buff,
                                   const KT_Socket::addr_type &addr = KT_Socket::addr_type());
  ...
  /*
     ...
     * @return int throw KT_Transceiver_Exception
     */
  void doRequest();

  /*
     * 接受网络请求
     * @return bool false: 对端关闭连接, true: 正常, throw KT_Transceiver_Exception
     */
  ...
  /**
     * 获取openssl
     */
  inline const std::shared_ptr<KT_OpenSSL> &getOpenSSL() { return _openssl; }
  ...
  /**
	 ...
	 * @return
	 */
  inline const KT_Endpoint &getConnectEndpoint() const { return _proxyInfo ? _proxyInfo->getEndpoint() : _ep; }

  /**
	 * 获取连接地址
	 * @return
	 */
  inline KT_Endpoint &getConnectEndpoint() { return _proxyInfo ? _proxyInfo->getEndpoint() : _ep; }
 ...
 protected:
  ...
  /**
     * 验证鉴权信息
     */
  void doAuthCheck(KT_NetWorkBuffer *buff);

  /**
     * 解析协议 
     */
  int doProtocolAnalysis(KT_NetWorkBuffer *buff);
 ...
 protected:
  /*
     * 服务端的地址
     */
  KT_Socket::addr_type _serverAddr;

  /**
     * 客户端地址
     */
  KT_Socket::addr_type _clientAddr;

  /**
     * bind addr
     */
  KT_Socket::addr_type _bindAddr;
  ...
  /*
     * 事件注册信息
     */
  shared_ptr<KT_Epoller::EpollInfo> _epollInfo;
  ...
  /*
     * openssl
     */
  std::shared_ptr<KT_OpenSSL> _openssl;

  /*
     * 发送buffer
     */
  KT_NetWorkBuffer _sendBuffer;

  /*
     * 接收buffer
     */
  KT_NetWorkBuffer _recvBuffer;

  /**
	 * 代理
	 */
  shared_ptr<KT_ProxyInfo> _proxyInfo;
  ...
  /**
     * 最后一个包的发送地址
     */
  KT_Socket::addr_type _lastAddr;

  /*
     * 接收缓存(udp情况才有效)
     */
  shared_ptr<KT_NetWorkBuffer::Buffer> _pRecvBuffer;
 ...
 * TCP 传输实现
 */
class KT_TCPTransceiver : public KT_Transceiver {
 public:
  /**
     * 构造函数
     * @param ep
     * @param fd
     */
  KT_TCPTransceiver(KT_Epoller *epoller, const KT_Endpoint &ep);
 ...
 * SSL 传输实现
 */
class KT_SSLTransceiver : public KT_TCPTransceiver {
 public:
  /**
     * 构造函数
     * @param ep
     * @param fd
     */
  KT_SSLTransceiver(KT_Epoller *epoller, const KT_Endpoint &ep);
 ...
 * UDP 传输实现
 */
class KT_UDPTransceiver : public KT_Transceiver {
 public:
  /**
     * 构造函数
     */
  KT_UDPTransceiver(KT_Epoller *epoller, const KT_Endpoint &ep);
