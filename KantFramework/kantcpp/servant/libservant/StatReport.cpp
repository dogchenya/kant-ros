...
void StatReport::setReportInfo(const StatFPrx& statPrx, const PropertyFPrx& propertyPrx, const string& strModuleName,
                               const string& strModuleIp, const string& strSetDivision, int iReportInterval,
                               int iSampleRate, unsigned int iMaxSampleCount, int iMaxReportSize, int iReportTimeout) {
  Lock lock(*this);
  ...
  vector<string> vtSetInfo = KT_Common::sepstr<string>(strSetDivision, ".");
...
string StatReport::trimAndLimitStr(const string& str, uint32_t limitlen) {
  ...
  string ret = KT_Common::trim(str, strTime);
...
bool StatReport::divison2SetInfo(const string& str, vector<string>& vtSetInfo) {
  vtSetInfo = KT_Common::sepstr<string>(str, ".");
...
void StatReport::report(const string& strModuleName, const string& setdivision, const string& strInterfaceName,
                        const string& strModuleIp, uint16_t iPort, StatResult eResult, int iSptime, int iReturnValue,
                        bool bFromClient) {
  //包头信息,trim&substr 防止超长导致udp包发送失败
  //masterIp为空服务端自己获取。
  ...
  if (bFromClient) {
    if (!_setName.empty()) {
      head.masterName = _moduleName + "." + _setName + _setArea + _setID + "@" + ClientConfig::KantVersion;
    } else {
      head.masterName = _moduleName + "@" + ClientConfig::KantVersion;
    }
    ...
    if (!setdivision.empty())  //被调没有启用set分组,slavename保持原样
    {
      ...
      head.slaveName = trimAndLimitStr(strModuleName, MAX_MASTER_NAME_LEN) + "." + head.slaveSetName +
                       head.slaveSetArea + head.slaveSetID;
    } else {
...
void StatReport::report(const string& strMasterName, const string& strMasterIp, const string& strSlaveName,
                        const string& strSlaveIp, uint16_t iSlavePort, const string& strInterfaceName,
  ...
  head.masterName = trimAndLimitStr(strMasterName + "@" + ClientConfig::KantVersion, MAX_MASTER_NAME_LEN);
...
int StatReport::reportMicMsg(MapStatMicMsg& msg, bool bFromClient) {
  ...
  try {
    ...
    TLOGKANT("[StatReport::reportMicMsg get size:" << mStatMsg.size() << "]" << endl);
    for (MapStatMicMsg::iterator it = mStatMsg.begin(); it != mStatMsg.end(); it++) {
      const StatMicMsgHead& head = it->first;
      int iTemLen = STAT_PROTOCOL_LEN + head.masterName.length() + head.slaveName.length() +
                    head.interfaceName.length() + head.slaveSetName.length() + head.slaveSetArea.length() +
                    head.slaveSetID.length();
      iLen = iLen + iTemLen;
      if (iLen > _maxReportSize)  //不能超过udp 1472
      {
        if (_statPrx) {
          TLOGKANT("[StatReport::reportMicMsg send size:" << mTemp.size() << "]" << endl);
          _statPrx->kant_set_timeout(_reportTimeout)
            ->async_reportMicMsg(NULL, mTemp, bFromClient, ServerConfig::Context);
        }
        iLen = iTemLen;
        mTemp.clear();
      }
      ...
      if (LOG->isNeedLog(LocalRollLogger::INFO_LOG)) {
        ...
        TLOGKANT("[StatReport::reportMicMsg display:" << os.str() << "]" << endl);
      }
    }
    if (0 != (int)mTemp.size()) {
      if (_statPrx) {
        TLOGKANT("[StatReport::reportMicMsg send size:" << mTemp.size() << "]" << endl);
        _statPrx->kant_set_timeout(_reportTimeout)->async_reportMicMsg(NULL, mTemp, bFromClient, ServerConfig::Context);
      }
    }
    return 0;
  } catch (exception& e) {
...
int StatReport::reportPropMsg() {
  try {
...

    {
      ...
      for (map<string, PropertyReportPtr>::iterator it = _statPropMsg.begin(); it != _statPropMsg.end(); ++it) {
        ...
        if (LOG->isNeedLog(LocalRollLogger::INFO_LOG)) {
          ...
          TLOGKANT("[StatReport::reportPropMsg display:" << os.str() << "]" << endl);
        }
      }
    }

    TLOGKANT("[StatReport::reportPropMsg get size:" << mStatMsg.size() << "]" << endl);
    int iLen = 0;
    ...
    for (MapStatPropMsg::iterator it = mStatMsg.begin(); it != mStatMsg.end(); it++) {
      ...
      int iTemLen = head.moduleName.length() + head.ip.length() + head.propertyName.length() + head.setName.length() +
                    head.setArea.length() + head.setID.length();
      ...
      {
        if (_propertyPrx) {
          TLOGKANT("[StatReport::reportPropMsg send size:" << mTemp.size() << "]" << endl);
          _propertyPrx->kant_set_timeout(_reportTimeout)->async_reportPropMsg(NULL, mTemp, ServerConfig::Context);
        }
        iLen = iTemLen;
        mTemp.clear();
      }
...
void StatReport::addMicMsg(MapStatMicMsg& old, MapStatMicMsg& add) {
  ...
  for (; iter != add.end(); ++iter) {
    ...
    if (iterOld == old.end()) {
      ...
      if (iterOld->second.maxRspTime < iter->second.maxRspTime) iterOld->second.maxRspTime = iter->second.maxRspTime;

      if (iterOld->second.minRspTime > iter->second.minRspTime) iterOld->second.minRspTime = iter->second.minRspTime;
    }
  }
}
