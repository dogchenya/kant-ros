...
{
  string s =
    string("GET /a/b?name=value&ccc=ddd HTTP/1.1\r\n") +
...
TEST_F(UtilHttpTest, testEncodeString)  //此时使用的是TEST_F宏
{
  ...
  cout << "testEncodeString::cost: " << KT_Common::now2us() - t << "us, " << 1. * (KT_Common::now2us() - t) / count
       << "us" << endl;
}
...
TEST_F(UtilHttpTest, testEncodeVector)  //此时使用的是TEST_F宏
{
  ...
  cout << "testEncodeVector::cost: " << KT_Common::now2us() - t << "us, " << 1. * (KT_Common::now2us() - t) / count
       << "us" << endl;
}
...
TEST_F(UtilHttpTest, testEncodeBuffString)  //此时使用的是TEST_F宏
{
  ...
  cout << "testEncodeBuffString::cost: " << KT_Common::now2us() - t << "us, " << 1. * (KT_Common::now2us() - t) / count
       << "us" << endl;
}
...
TEST_F(UtilHttpTest, testDecodeString)  //此时使用的是TEST_F宏
{
  ...
  cout << "testDecodeString::cost: " << KT_Common::now2us() - t << "us, " << 1. * (KT_Common::now2us() - t) / count
       << "us" << endl;
}
...
TEST_F(UtilHttpTest, testDecodeBuffString)  //此时使用的是TEST_F宏
{
  ...
  cout << "testDecodeBuffString::cost: " << KT_Common::now2us() - t << "us, " << 1. * (KT_Common::now2us() - t) / count
       << "us" << endl;
}
...
TEST_F(UtilHttpTest, testCheckRequestString)  //此时使用的是TEST_F宏
{
  ...
  cout << "testCheckRequestString::cost: " << KT_Common::now2us() - t << "us, "
       << 1. * (KT_Common::now2us() - t) / count << "us" << endl;
}
...
TEST_F(UtilHttpTest, testCheckRequestBuff)  //此时使用的是TEST_F宏
{
  ...
  cout << "testCheckRequestBuff::cost: " << KT_Common::now2us() - t << "us, " << 1. * (KT_Common::now2us() - t) / count
       << "us" << endl;
}
...
TEST_F(UtilHttpTest, testHttpFinish)  //此时使用的是TEST_F宏
{
  ...
  sbuff.push_back("Content-Length: " + KT_Common::tostr(body.size()) + "\r\n");
  ...
  ASSERT_TRUE(buff.checkHttp() == KT_NetWorkBuffer::PACKET_FULL);
}
...
TEST_F(UtilHttpTest, testHttpNoFinish)  //此时使用的是TEST_F宏
{
  ...
  sbuff.push_back("Accept-Charset: utf-8,gb2321;q=0.7,*;q=0.7\r\n");
  sbuff.push_back("Accept-Encoding: gzip\r\n");
  sbuff.push_back("Content-Length: " + KT_Common::tostr(body.size() + 1) + "\r\n");
  sbuff.push_back("Connection: close\r\n");
  sbuff.push_back("User-Agent: E71/SymbianOS/9.1 Series60/3.0\r\n\r\n");
  ...
  ASSERT_TRUE(buff.checkHttp() == KT_NetWorkBuffer::PACKET_LESS);
}
...
void testCookie(const string &sRspURL, const string &sReqURL, const vector<string> &vsCookie) {
  ...
  KT_HttpCookie cookie;

  cookie.addCookie(sRspURL, vsCookie);

  // list<KT_HttpCookie::Cookie> vCookie = cookie.getAllCookie();

  // list<KT_HttpCookie::Cookie>::iterator it = vCookie.begin();

  // while(it != vCookie.end())
  // {
  // 	cout << KT_Common::tostr(it->_data.begin(), it->_data.end(), "; ") << ", " << it->_expires << ", " << it->_path << endl;
  ...
  //	cout << KT_Common::tostr(sCookie) << endl;
...
{
  ASSERT_TRUE(KT_HttpCookie::matchDomain("qq.com", "www.qq.com") == true);
  ASSERT_TRUE(KT_HttpCookie::matchDomain(".qq.com", "www.qq.com") == true);
  ASSERT_TRUE(KT_HttpCookie::matchDomain(".qq.com", "qq.com") == true);
  ASSERT_TRUE(KT_HttpCookie::matchDomain("t.qq.com", "www.qq.com") == false);
  ASSERT_TRUE(KT_HttpCookie::matchDomain(".t.qq.com", "www.qq.com") == false);
  ASSERT_TRUE(KT_HttpCookie::matchDomain(".t.qq.com", "t.qq.com") == true);
  ASSERT_TRUE(KT_HttpCookie::matchDomain(".com", "www.qq.com") == false);
  ASSERT_TRUE(KT_HttpCookie::matchDomain(".com", "qq.com") == false);
  ASSERT_TRUE(KT_HttpCookie::matchDomain(".y.qq.com", "x.y.qq.com") == true);
  ASSERT_TRUE(KT_HttpCookie::matchDomain(".x.y.qq.com", "x.y.qq.com") == true);
  ASSERT_TRUE(KT_HttpCookie::matchDomain(".qq.com", "x.y.qq.com") == true);
  ASSERT_TRUE(KT_HttpCookie::matchDomain(".qq.com", "y.qq.com") == true);
  ASSERT_TRUE(KT_HttpCookie::matchDomain("qq.com", "y.qq.com") == true);

  //	cout << KT_Common::now2GMTstr() << endl;

  string gmt = KT_Common::tm2GMTstr(time(NULL) + 10);
  ...
  //	cout << KT_Common::tostr(vsCookie.begin(), vsCookie.end(), "\r\n") << endl << endl;
...
TEST_F(UtilHttpTest, testBaidu)  //此时使用的是TEST_F宏
{
  ...
  //	cout << KT_Common::tostr(headers.begin(), headers.end(), "\r\n") << endl;
  //	KT_File::save2file(file, stHttpRsp.getContent());
...
TEST_F(UtilHttpTest, testIncrementDecode1) {
  ...
  string data =
    "ddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd-0-"
    "139896025024448";
  string s = "HTTP/1.1 200 OK\r\n";
  s += "Content-Length : 115\r\n\r\n";
  s += data;

  KT_NetWorkBuffer::Buffer buff;
  buff.addBuffer(s);
