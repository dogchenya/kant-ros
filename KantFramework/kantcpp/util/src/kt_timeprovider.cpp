...
KT_TimeProvider* KT_TimeProvider::g_tp = NULL;

KT_TimeProvider* KT_TimeProvider::getInstance() {
  static std::once_flag flag;
  std::call_once(flag, []() {
    g_tp = new KT_TimeProvider();
    g_tp->start();
  });
...
KT_TimeProvider::~KT_TimeProvider() {}

#if TARGET_PLATFORM_IOS || TARGET_PLATFORM_LINUX

//直接从寄存器获取开机上电以来的周期(和主频有关系)
//如果微处理器的主频是1MHZ的话，那么tsc就会在1秒内增加1000000
uint64_t KT_TimeProvider::GetCycleCount() {
...
void KT_TimeProvider::getNow(timeval* tv) {
#if TARGET_PLATFORM_IOS || TARGET_PLATFORM_LINUX
...
uint64_t KT_TimeProvider::getNowMs() {
  struct timeval tv;
  getNow(&tv);
...
void KT_TimeProvider::terminate() {
  _terminate = true;
...
void KT_TimeProvider::setTsc(timeval& tt) {
...
void KT_TimeProvider::addTimeOffset(timeval& tt, const int& idx) {
  ...
  if (t < -1000 || offset > 1000000) {
    cerr << "KT_TimeProvider add_time_offset error,correct it by use gettimeofday. offset:" << offset
         << "|current_tsc:" << current_tsc << "|last_tsc:" << _tsc[idx] << "|t:" << t << endl;
    _use_tsc = false;
    KT_Common::gettimeofday(tt);
    return;
  }
