...
* 编解码抛出的异常
*/
struct KT_Xml_Exception : public KT_Exception {
  KT_Xml_Exception(const string& buffer) : KT_Exception(buffer){};
  KT_Xml_Exception(const string& buffer, int err) : KT_Exception(buffer, err){};
  ~KT_Xml_Exception() throw(){};
};
...
class XmlValueString : public XmlValue {
 public:
  ...
  eXmlType getType() { return eXmlTypeString; }
  virtual ~XmlValueString() {}
  string value;
  bool cdata;
};
typedef std::shared_ptr<XmlValueString> XmlValueStringPtr;
...
class XmlValueObj : public XmlValue {
 public:
  eXmlType getType() { return eXmlTypeObj; }
  virtual ~XmlValueObj() {}

 public:
  map<string, XmlValuePtr> value;
};
typedef std::shared_ptr<XmlValueObj> XmlValueObjPtr;
...
class XmlValueArray : public XmlValue {
 public:
  eXmlType getType() { return eXmlTypeArray; }
  void push_back(XmlValuePtr& p) { value.push_back(p); }
  virtual ~XmlValueArray() {}

 public:
  vector<XmlValuePtr> value;
};
typedef std::shared_ptr<XmlValueArray> XmlValueArrayPtr;
...
class BufferXmlReader {
 ...
 public:
  ...
  bool hasEnd() { return (_pos >= _len || *(_buf + _pos) == 0); }
