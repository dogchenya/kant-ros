...
KT_ThreadLock DbProxy::_mutex;

map<string, NodePrx> DbProxy::_mapNodePrxCache;

KT_ThreadLock DbProxy::_NodePrxLock;
vector<map<string, string>> DbProxy::_serverGroupRule;

//key-ip, value-组编号
map<string, int> DbProxy::_serverGroupCache;

vector<kant::KT_Mysql*> DbProxy::_mysqlReg;
vector<KT_ThreadMutex*> DbProxy::_mysqlLocks;
...
#define MYSQL_LOCK             \
  size_t nowIndex = ++g_index; \
  KT_LockT<KT_ThreadMutex> Lock(*(_mysqlLocks[nowIndex % _mysqlLocks.size()]));
#define MYSQL_INDEX _mysqlReg[nowIndex % _mysqlReg.size()]
int DbProxy::init(KT_Config* pconf) {
  try {
    int connNums = KT_Common::strto<int>(pconf->get("/kant/db<conn_num>", "5"));
    connNums = (connNums < 1 ? 5 : connNums);
    connNums = (connNums > 100 ? 5 : connNums);
    TLOG_DEBUG("connNums=" << connNums << endl);

    KT_DBConf tcDBConf;
    tcDBConf.loadFromMap(pconf->getDomainMap("/kant/db"));
    TLOG_DEBUG("mysql param:" << KT_Common::tostr(pconf->getDomainMap("/kant/db")) << endl);

    for (int i = 0; i < connNums; ++i) {
      KT_Mysql* pMysql = new KT_Mysql(tcDBConf);
      _mysqlReg.push_back(pMysql);
      _mysqlLocks.push_back(new KT_ThreadMutex());
    }
    ...
    _patchHistory = KT_Common::strto<int>(pconf->get("/kant/patch<patchHistory>", "200"));
    if (_patchHistory < 10) {
      _patchHistory = 10;
    }
...
int DbProxy::addTaskReq(const TaskReq& taskReq) {
  try {
    for (size_t i = 0; i < taskReq.taskItemReq.size(); i++) {
      //不是发布, 不需要记录
      if (taskReq.taskItemReq[i].command != "patch_kant" && taskReq.taskItemReq[i].command != "grace_patch_kant")
        return 0;
    }

    for (size_t i = 0; i < taskReq.taskItemReq.size(); i++) {
      map<string, pair<KT_Mysql::FT, string>> data;
      data["task_no"] = make_pair(KT_Mysql::DB_STR, taskReq.taskNo);
      data["item_no"] = make_pair(KT_Mysql::DB_STR, taskReq.taskItemReq[i].itemNo);
      data["application"] = make_pair(KT_Mysql::DB_STR, taskReq.taskItemReq[i].application);
      data["server_name"] = make_pair(KT_Mysql::DB_STR, taskReq.taskItemReq[i].serverName);
      data["node_name"] = make_pair(KT_Mysql::DB_STR, taskReq.taskItemReq[i].nodeName);
      data["command"] = make_pair(KT_Mysql::DB_STR, taskReq.taskItemReq[i].command);
      data["parameters"] = make_pair(KT_Mysql::DB_STR, KT_Parsepara(taskReq.taskItemReq[i].parameters).tostr());
...

    {
      map<string, pair<KT_Mysql::FT, string>> data;
      data["task_no"] = make_pair(KT_Mysql::DB_STR, taskReq.taskNo);
      data["serial"] = make_pair(KT_Mysql::DB_INT, KT_Common::tostr(taskReq.serial));
      data["create_time"] = make_pair(KT_Mysql::DB_INT, "now()");
      data["user_name"] = make_pair(KT_Mysql::DB_STR, taskReq.userName);
...
int DbProxy::getTaskRsp(const string& taskNo, TaskRsp& taskRsp) {
  try {
    ...
    taskRsp.serial = KT_Common::strto<int>(item[0]["serial"]);
    ...
    for (unsigned i = 0; i < item.size(); i++) {
      ...
      rsp.status = (EMTaskItemStatus)KT_Common::strto<int>(item[i]["status"]);
      ...
      rsp.req.parameters = KT_Parsepara(item[i]["parameters"]).toMap();
...
int DbProxy::getTaskHistory(const string& application, const string& serverName, const string& command,
                            vector<TaskRsp>& taskRsp) {
  try {
    KT_Mysql::MysqlData res;

    {
      MYSQL_LOCK;
      string sSql =
        "select t1.`create_time`, t1.`serial`, t1.`user_name`, t2.* from t_task as t1, t_task_item as t2 where "
        "t1.task_no=t2.task_no and t2.application='" +
        MYSQL_INDEX->escapeString(application) + "' and t2.server_name='" + MYSQL_INDEX->escapeString(serverName) +
        "' and t2.command='" + MYSQL_INDEX->escapeString(command) + "' order by create_time desc, task_no";
    ...
    for (unsigned i = 0; i < res.size(); i++) {
      ...
      itemRsp.status = (EMTaskItemStatus)KT_Common::strto<int>(res[i]["status"]);
      ...
      itemRsp.req.parameters = KT_Parsepara(res[i]["parameters"]).toMap();

      if (taskRsp.empty() || taskRsp[taskRsp.size() - 1].taskNo != taskNo) {
        //新的TaskRsp
        TaskRsp rsp;
        rsp.taskNo = res[i]["task_no"];
        rsp.serial = KT_Common::strto<int>(res[i]["serial"]);
...
int DbProxy::setTaskItemInfo(const string& itemNo, const map<string, string>& info) {
  ...
  try {
    map<string, pair<KT_Mysql::FT, string>> data;
    data["item_no"] = make_pair(KT_Mysql::DB_STR, itemNo);
    map<string, string>::const_iterator it = info.find("start_time");
    if (it != info.end()) {
      data["start_time"] = make_pair(KT_Mysql::DB_STR, it->second);
    }
    it = info.find("end_time");
    if (it != info.end()) {
      data["end_time"] = make_pair(KT_Mysql::DB_STR, it->second);
    }
    it = info.find("status");
    if (it != info.end()) {
      data["status"] = make_pair(KT_Mysql::DB_INT, it->second);
    }
    it = info.find("log");
    if (it != info.end()) {
      data["log"] = make_pair(KT_Mysql::DB_STR, it->second);
    }
...
int DbProxy::undeploy(const string& application, const string& serverName, const string& nodeName, const string& user,
                      string& log) {
  string where =
    "where application='" + application + "' and server_name='" + serverName + "' and node_name='" + nodeName + "'";
...
int DbProxy::setPatchInfo(const string& app, const string& serverName, const string& nodeName, const string& version,
                          const string& user) {
  try {
    MYSQL_LOCK;
    string sSql =
      ...
      "set patch_version = '" +
      MYSQL_INDEX->escapeString(version) +
      "', "
      "   patch_user = '" +
      MYSQL_INDEX->escapeString(user) +
      "', "
      "   patch_time = now() "
      "where application='" +
      MYSQL_INDEX->escapeString(app) +
      "' "
      "    and server_name='" +
      MYSQL_INDEX->escapeString(serverName) +
      "' "
      "    and node_name='" +
      MYSQL_INDEX->escapeString(nodeName) + "' ";
    ...
    TLOG_ERROR("DbProxy::setPatchInfo " << app << "." << serverName << "_" << nodeName << " exception: " << ex.what()
                                        << endl);
    return -1;
  }
}
...
int DbProxy::getNodeVersion(const string& nodeName, string& version, string& result) {
  try {
    MYSQL_LOCK;
    string sSql =
      "select kant_version from t_node_info "
      "where node_name='" +
      MYSQL_INDEX->escapeString(nodeName) + "'";

    KT_Mysql::MysqlData res = MYSQL_INDEX->queryRecord(sSql);
    TLOG_DEBUG(__FUNCTION__ << " (node_name='" << nodeName << "') affected:" << res.size() << endl);
    if (res.size() > 0) {
      version = res[0]["kant_version"];
      return 0;
    }
...
int DbProxy::updateServerState(const string& app, const string& serverName, const string& nodeName,
                               const string& stateFields, kant::ServerState state, int processId) {
  try {
    int64_t iStart = KT_TimeProvider::getInstance()->getNowMs();
    if (stateFields != "setting_state" && stateFields != "present_state") {
      TLOG_DEBUG(app << "." << serverName << "_" << nodeName << " not supported fields:" << stateFields << endl);
      return -1;
    }

    string sProcessIdSql =
      (stateFields == "present_state" ? (", process_id = " + KT_Common::tostr<int>(processId) + " ") : "");

    MYSQL_LOCK;
    string sSql =
      "update t_server_conf "
      "set " +
      stateFields + " = '" + etos(state) + "' " + sProcessIdSql + "where application='" +
      MYSQL_INDEX->escapeString(app) +
      "' "
      "    and server_name='" +
      MYSQL_INDEX->escapeString(serverName) +
    ...
    TLOG_DEBUG(__FUNCTION__ << " " << app << "." << serverName << "_" << nodeName
                            ...
                            << "|cost:" << (KT_TimeProvider::getInstance()->getNowMs() - iStart) << endl);
    return 0;

  } catch (KT_Mysql_Exception& ex) {
    TLOG_ERROR(__FUNCTION__ << " " << app << "." << serverName << "_" << nodeName << " exception: " << ex.what()
                            << endl);
    return -1;
  }
}

int DbProxy::gridPatchServer(const string& app, const string& servername, const string& nodename,
                             const string& status) {
  try {
    MYSQL_LOCK;
...
vector<ServerDescriptor> DbProxy::getServers(const string& app, const string& serverName, const string& nodeName,
                                             bool withDnsServer) {
  string sSql;
  ...
  try {
    KT_Mysql::MysqlData res;
    {
      ...
      if (withDnsServer == false) sCondition += " and server.server_type !='kant_dns' ";  //不获取dns服务
    ...
    for (unsigned i = 0; i < res.size(); i++) {
      ...
      if (mapAppServerTemp.find(sServerId) == mapAppServerTemp.end()) {
        ...
        server.configCenterPort = KT_Common::strto<int>(res[i]["config_center_port"]);
        server.runType = res[i]["run_type"];
        //                server.bakFlag      = KT_Common::strto<int>(res[i]["bak_flag"]);

        server.setId = "";
        if (KT_Common::lower(res[i]["enable_set"]) == "y") {
        ...
        KT_Config tParent, tProfile;
        tParent.parseString(mapProfile[res[i]["template_name"]]);
        tProfile.parseString(server.profile);
        int iDefaultAsyncThreadNum = 3;
        if (server.serverType == "kant_nodejs") {
          iDefaultAsyncThreadNum = 0;
        }
        int iConfigAsyncThreadNum = KT_Common::strto<int>(KT_Common::trim(res[i]["async_thread_num"]));
        iDefaultAsyncThreadNum =
          iConfigAsyncThreadNum > iDefaultAsyncThreadNum ? iConfigAsyncThreadNum : iDefaultAsyncThreadNum;
        server.asyncThreadNum = KT_Common::strto<int>(
          tProfile.get("/kant/application/client<asyncthread>", KT_Common::tostr(iDefaultAsyncThreadNum)));
      ...
      adapter.maxConnections = KT_Common::strto<int>(res[i]["max_connections"]);
      adapter.allowIp = res[i]["allow_ip"];
      adapter.servant = res[i]["servant"];
      adapter.queuecap = KT_Common::strto<int>(res[i]["queuecap"]);
      adapter.queuetimeout = KT_Common::strto<int>(res[i]["queuetimeout"]);
    ...
    TLOG_ERROR(__FUNCTION__ << " " << app << "." << serverName << "_" << nodeName
                            << " KT_Config_Exception exception: " << ex.what() << endl);
    throw KantException(string("KT_Config_Exception exception: ") + ex.what());
  }

  TLOG_DEBUG(__FUNCTION__ << " " << app << "." << serverName << "_" << nodeName << " getServers affected:" << num
                          << "|cost:" << (KT_TimeProvider::getInstance()->getNowMs() - iStart) << endl);
...
int DbProxy::getNodeList(const string& app, const string& serverName, vector<string>& nodeNames) {
  ...
  try {
    MYSQL_LOCK;
    string sSql = "select node_name from t_server_conf where application='" + MYSQL_INDEX->escapeString(app) +
...
string DbProxy::getProfileTemplate(const string& sTemplateName, map<string, int>& mapRecursion, string& sResultDesc) {
  try {
    KT_Mysql::MysqlData res;
    {
      MYSQL_LOCK;
      string sSql =
        "select template_name, parents_name, profile from t_profile_template "
        "where template_name='" +
        MYSQL_INDEX->escapeString(sTemplateName) + "'";
    ...
    KT_Config confMyself, confParents;
    ...
    sResultDesc += "(" + sTemplateName + ":" + ex.what() + ")";
    TLOG_ERROR(__FUNCTION__ << " KT_Config_Exception exception: " << ex.what() << endl);
  }
...
int DbProxy::getGroupId(const string& ip) {
  ...
  try {
    {
      KT_ThreadLock::Lock lock(_mutex);
    ...
    for (unsigned i = 0; i < vServerGroupInfo.size(); i++) {
      iGroupId = KT_Common::strto<int>(vServerGroupInfo[i]["group_id"]);
      ...
      vector<string> vAllowIp = KT_Common::sepstr<string>(sAllowIpRule, ",|;");
      vector<string> vDennyIp = KT_Common::sepstr<string>(sDennyIpRule, ",|;");
      if (sOrder == "allow_denny") {
        if (KT_Common::matchPeriod(ip, vAllowIp)) {
          bFind = true;
          break;
        }
      } else if (sOrder == "denny_allow") {
        if (KT_Common::matchPeriod(ip, vDennyIp)) {
    ...
    if (bFind == true) {
      ...
      TLOGINFO("get groupId succ|ip|" << ip << "|group_id|" << iGroupId << "|ip_order|" << sOrder << "|allow_ip_rule|"
                                      << sAllowIpRule << "|denny_ip_rule|" << sDennyIpRule << "|ServerGroupCache|"
                                      << KT_Common::tostr(_serverGroupCache) << endl);
...
NodePrx DbProxy::getNodePrx(const string& nodeName) {
  try {
    KT_ThreadLock::Lock lock(_NodePrxLock);
    ...
    KT_Mysql::MysqlData res;
    {
      ...
      string sSql =
        ...
        "from t_node_info "
        "where node_name='" +
        MYSQL_INDEX->escapeString(nodeName) + "' and present_state='active'";
    ...
    if (res.size() == 0) {
      throw KantNodeNotRegistryException("node '" + nodeName +
                                         "' not registered  or heartbeart timeout,please check for it");
    }
    ...
    throw KantNodeNotRegistryException(string("get node record from db error:") + ex.what());
  } catch (kant::KantException& ex) {
    TLOG_ERROR(__FUNCTION__ << " " << nodeName << " exception: " << ex.what() << endl);
    throw ex;
  }
}

int DbProxy::getFramework(vector<kant::FrameworkServer>& servers) {
  try {
    MYSQL_LOCK;
    string sSql = "select * from t_adapter_conf where application = 'kant'";

    KT_Mysql::MysqlData data = MYSQL_INDEX->queryRecord(sSql);
...
int DbProxy::checkRegistryTimeout(unsigned uTimeout) {
  try {
    ...
    string sSql =
      ...
      kant::KT_Common::tostr(uTimeout) + " SECOND)";
...
int DbProxy::updateRegistryInfo2Db(bool bRegHeartbeatOff) {
  ...
  try {
    MYSQL_LOCK;
    string sSql =
      "replace into t_registry_info (locator_id, servant, endpoint, last_heartbeat, present_state, kant_version)  "
      "values ";

    string sVersion = Application::getKantVersion() + "_" + SERVER_VERSION;
    for (iter = mapServantEndpoint.begin(); iter != mapServantEndpoint.end(); iter++) {
      KT_Endpoint locator;
      locator.parse(iter->second);

      sSql += (iter == mapServantEndpoint.begin() ? string("") : string(", ")) + "('" + locator.getHost() + ":" +
              KT_Common::tostr<int>(locator.getPort()) +
              "', "
              "'" +
              iter->first + "', '" + iter->second + "', now(), 'active', " + "'" + MYSQL_INDEX->escapeString(sVersion) +
              "')";
    }
...
int DbProxy::loadIPPhysicalGroupInfo() {
  try {
    KT_Mysql::MysqlData res;
    {
      MYSQL_LOCK;
      string sSql =
        "select group_id,ip_order,allow_ip_rule,denny_ip_rule,group_name from t_server_group_rule "
        "order by group_id";
      res = MYSQL_INDEX->queryRecord(sSql);
    }
    if (res.size() > 0) {
...
int DbProxy::updatePatchByPatchId(const string& application, const string& serverName, const string& nodeName,
                                  const string& patchId, const string& user, const string& patchType, bool succ) {
  try {
    MYSQL_LOCK;
    string sql = "update t_server_patchs set publish='1',publish_user='" + user + "',publish_time=now(),lastuser='" +
                 user + "' where id=" + patchId;
...
vector<string> DbProxy::deleteHistorys(const string& application, const string& serverName) {
  ...
  try {
    ...
    string sql = "select id from t_server_patchs where " + where + " order by id desc limit " +
                 KT_Common::tostr(_patchHistory) + ", 1";

    auto data = MYSQL_INDEX->queryRecord(sql);

    if (data.size() > 0) {
      auto dels =
        MYSQL_INDEX->queryRecord("select tgz from t_server_patchs where " + where + " and id < " + data[0]["id"]);
...
int DbProxy::updateServerFlowState(const string& app, const string& serverName, const vector<string>& nodeList,
                                   bool bActive) {
  try {
    int64_t iStart = KT_TimeProvider::getInstance()->getNowMs();
    ...
    TLOG_ERROR(__FUNCTION__ << " " << app << "." << serverName << "_" << KT_Common::tostr(nodeList)
                            << " exception: " << ex.what() << endl);
  }
  return -1;
}

int DbProxy::updateServerFlowStateOne(const string& app, const string& serverName, const string& nodeName,
                                      bool bActive) {
  try {
    int64_t iStart = KT_TimeProvider::getInstance()->getNowMs();
    ...
    TLOG_DEBUG(__FUNCTION__ << " " << app << "." << serverName << "_" << nodeName << " affected:" << ar
                            << "|cost:" << (KT_TimeProvider::getInstance()->getNowMs() - iStart) << endl);
    return (int)ar;
  } catch (KT_Mysql_Exception& ex) {
    TLOG_ERROR(__FUNCTION__ << " " << app << "." << serverName << "_" << nodeName << " exception: " << ex.what()
