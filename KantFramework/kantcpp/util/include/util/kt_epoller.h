#ifndef _KT_EPOLLER_H_
#define _KT_EPOLLER_H_
 ...
 * 使用说明如下:
 * 1 KT_Epoller协程配合, 即kt_coroutine底层其实有一个epoller对象, 协程的切换都是通过epoller来进行的, 之所以这样设计是为了保证协程中网络IO调用的时候能顺利完成协程的切换
 * 2 KT_Epoller继承至KT_TimerBase, 即它本身也是一个定时器, 即可以用做timer
 * 3 KT_Epoller需要调用create来完成初始化
 * 4 KT_Epoller中有一个子类EpollInfo, 该子类非常重要, 当需要用epoll操控某个句柄是, 你可以通过epoll.createEpollInfo创建对象
 ...
 * 9 KT_Epoller对象的loop方法, 会发起一个epoll wait的事件循环, 会阻塞当前线程
 * 10 KT_Epoller对象的done方法, 会执行一次epoll wait事件, 如果没有任何事件发生, 则只会等待最后ms毫秒(参数确定)
 * 11 KT_Epoller对象中的notify方法, 可以主动唤醒epoll wait
 */
/////////////////////////////////////////////////
 ...
 */
class KT_Epoller : public KT_TimerBase {
 public:
  class EpollInfo : public enable_shared_from_this<EpollInfo> {
   public:
    /**
		 ...
		 * @param fd
		 */
    EpollInfo(KT_Epoller *epoller, int fd) : _epoller(epoller), _fd(fd) {}
    ...
    typedef std::function<bool(const shared_ptr<KT_Epoller::EpollInfo> &)> EVENT_CALLBACK;
    ...
    /**
		 ...
		 */
    void setCallback(const std::function<void(const shared_ptr<KT_Epoller::EpollInfo> &)> &callback) {
      _callback = callback;
    }
   ...
   protected:
    ...
    std::function<void(const shared_ptr<KT_Epoller::EpollInfo> &)> _callback;
  };
 ...
 public:
  ...
  /**
	 ...
	 * EpollInfo被KT_Epoller管理, 连接关闭时自动释放, 使用者需要保持EpollInfo的智能智能指针, 当fd关闭时, 调用releaseEpollInfo
     * @param fd    句柄
     * @return EpollInfo
	 */
