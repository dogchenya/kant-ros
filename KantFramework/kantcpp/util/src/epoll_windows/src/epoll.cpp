...
class epoll_port_data_t {
 ...
 protected:
  ...
  void add(epoll_sock_data_t *sock_data) { _sock_data_tree[sock_data->_sock] = sock_data; }
  void remove_attn(epoll_sock_data_t *sock_data) { _attn_list.erase(sock_data); }
  void add_attn(epoll_sock_data_t *sock_data) { _attn_list.insert(sock_data); }
...
SOCKET epoll_port_data_t::get_peer_socket(WSAPROTOCOL_INFOW *protocol_info) {
  ...
  for (int i = 0; i < ARRAY_COUNT(AFD_PROVIDER_IDS); i++) {
    if (memcmp((void *)&protocol_info->ProviderId, (void *)&AFD_PROVIDER_IDS[i], sizeof protocol_info->ProviderId) ==
        0) {
      index = i;
    }
  }
...
SOCKET epoll_port_data_t::create_peer_socket(WSAPROTOCOL_INFOW *protocol_info) {
  SOCKET sock = WSASocketW(protocol_info->iAddressFamily, protocol_info->iSocketType, protocol_info->iProtocol,
...
int epoll_port_data_t::epoll_close() {
  ...
  while (!_sock_data_tree.empty()) {
    ...
    for (ULONG i = 0; i < count; i++) {
      ...
      delete op;
      if (sock_data->_op_count == 0) delete sock_data;
    }
...
epoll_t epoll_create(int size) {
  ...
  if (!epoll__initialized) {
    if (epoll__initialize() < 0) return NULL;
...
int epoll__initialize() {
  ...
  if (pNtDeviceIoControlFile == NULL) return -1;
...
int epoll__afd_poll(SOCKET socket, AFD_POLL_INFO *info, OVERLAPPED *overlapped) {
  ...
  status = pNtDeviceIoControlFile((HANDLE)socket, event, NULL, apc_context, iosb_ptr, IOCTL_AFD_POLL, info,
